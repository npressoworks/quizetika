...
test.describe('コミュニティモデレーション・フィードバック・ガバナンス E2Eテスト', () => {
  ...
  test('NGワードを含むクイズタイトルの保存がブロックされること', async ({ page }) => {
    ...
    await page.locator('input[placeholder="例: 世界の国旗と首都クイズ"]').fill('スパムスパムスパムXXXX');
  ...
  test('ジャンル新設申請機能のアクセス制限とUI非表示の確認（一般：非表示・404、管理者：表示）', async ({ page }) => {
    ...
    if (!hasGenreLink) {
      ...
      // (タブボタン自体の文言「承認・否決履歴」は常時表示されているため、
      //  コンテンツ側の「空状態メッセージ」または「履歴テーブル」の表示で判定する)
      const historyArea = page.locator('text=まだ完了したジャンル申請はありません').first()
  ...
  test('管理者モデレーション画面にアクセスできること (管理者権限を持つユーザーのみ)', async ({ page }) => {
    ...
    // 2. 権限判定は user プロファイルの非同期読み込み完了後に行われるため、
    //    ページ遷移直後の単発スナップショットでは判定が間に合わないことがある。
    //    expect.poll で判定が収束するまで待ってから、いずれかの状態に達したことを確認する
    //    (通常ユーザーはアクセス拒否・リダイレクト、管理者ユーザーは画面表示となる)
    const accessDeniedLocator = page.locator('text=アクセス権限');
    const adminUILocator = page.locator('h1').first();

    await expect.poll(async () => {
      const isRedirected = !page.url().includes('/admin/moderation');
      const hasAccessDenied = await accessDeniedLocator.isVisible().catch(() => false);
      const hasAdminUI = await adminUILocator.isVisible().catch(() => false);
      return isRedirected || hasAccessDenied || hasAdminUI;
