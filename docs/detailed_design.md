...
sequenceDiagram
    ...
    alt 下書き保存の場合 (isPublished = false)
        ...
        QC->>QC: 設問数(>=1)、各設問正解設定、文字数制限などを検証
        ...
        alt バリデーション失敗
            ...
            alt NGワード検知 (true)
                ...
                activate QS
                QS->>DB: トランザクション開始
                DB->>DB: quizzes ドキュメント新規作成/更新 (非正規化 authorName, authorAvatar 同期)
                QS-->>QC: 保存完了 (quizId)
                deactivate QS
...
sequenceDiagram
    ...
    DB-->>AS: { aiQuestionsHistory, aiTurnCount, aiTurnLimit }
    ...
    alt aiQuestionsHistory 内に完全一致の questionText が存在する
        ...
        alt aiTurnCount >= aiTurnLimit (無料ユーザーの20ターン到達)
            ...
            DB->>DB: aiTurnCount を +1 アトミックインクリメント
...
| ユーザータイプ | `aiTurnLimit` | 制限挙動 |
|:---|:---|:---|
| 無料ユーザー | `20` | `aiTurnCount >= 20` で `limit-exceeded` エラー |
| 有料プランユーザー | `null` | 制限なし（上限チェックをスキップ） |

- `aiTurnCount` はキャッシュヒットでは増加しません（同じ質問を繰り返してもターンを消費しない）。
- `aiTurnLimit` はプレイ開始時（Attempt作成時）に、ユーザーのプラン情報を参照して設定されます。
...
### 3.6 アカウント削除時のデータ処理（ソフト論理削除・匿名化・クイズ存続） [NEW]
認証済みユーザーが退会（アカウント削除：F-106）した際、個人情報は物理削除されますが、コミュニティの活性化およびデータの整合性を担保するため、退会ユーザーが作成したクイズやリスト、および過去のアクティビティデータに対して**公開存続とソフト論理削除・匿名化**を適用します。

1. **個人情報の物理削除**:
   - `users` コレクションの該当ドキュメントから、メールアドレス (`email`) などの個人情報を削除、またはドキュメント自体を匿名化削除（`displayName = "退会済みユーザー"`, `avatarUrl = "default_url"`, `email = ""`）します。
2. **作成した公開クイズ・クイズリストの公開維持と匿名化（作成者不在状態での存続）**:
   - 退会ユーザーが作成したすべての公開クイズ (`quizzes`) およびリスト (`quizLists`) のステータスは、**一律 `status = 'published'` および `isPublished = true` のまま公開が維持されます（非公開化や他リストからの削除は行いません）**。
   - 退会処理時、アトミックなバッチ処理により、これらのクイズおよびリストドキュメント内の作成者情報フィールドを一括で書き換え、個人情報を含まない「作成者不在状態」にします。
     - `authorId` ──> `"deleted_user"`
     - `authorName` ──> `"退会済みユーザー"`
     - `authorAvatar` ──> システム規定の退会ユーザー用デフォルトアバターURL（または空文字など既定値）
   - 他のユーザーのクイズリスト（`quizLists.quizIds`）に退会者のクイズが含まれている場合も、自動削除されず、そのまま存続してプレイ可能です。
3. **間違い指摘レポート・通知・リアクションの匿名化（存続）**:
   - 過去に他のクイズ作家へ送信した指摘レポート (`feedbackReports`)、通知 (`notifications`)、およびリアクション (`reactions`) は、物理削除すると作家側の履歴や統計データが消失してしまうため、削除せずに存続させます。
   - レポートや通知、リアクション内の `reporterId`、`senderId`、または `receiverId`、および非正規化された `reporterName` / `senderName` などの情報を `"deleted_user"` および `"退会済みユーザー"` へ一括で書き換え、個人情報を一切残さずに履歴のみを維持します。
...
* **整形仕様**:
  ...
  - 半角スペース、全角スペース、および記号（`_`, `-`, `.`, `/` 等）を完全に排除 (`.replace(/[^a-z0-9]/g, '')`)
...
* **同一評価者からの加算上限制限**:
  ...
  - 日次再計算バッチにおいて、`users/{uid}/reputationEvents` サブコレクションから集計を行う際、同一 `senderId`からの加算分をグルーピングし、上限を超過したイベントは `processedAt` を記録しつつも `delta` を `0` として処理します。
...
match /quizzes/{quizId} {
  // reviewScore・reviewBadge・positiveCount・negativeCount はクライアントから直接変更不可
  allow update: if request.auth != null
