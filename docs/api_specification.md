...
| **`createQuiz`** | クイズを新規作成する（下書きまたは直接公開）。Zodスキーマ検証を通す。ユーザーが入力した生のタグ配列は `originalTags` にそのまま保存され、表記揺れを排除した標準表記タグID配列が `tags` に保存される。さらに仮想マージ解決適用後の統合先（正規）タグID配列が `canonicalTagIds` に保存される。**Phase 8**: `linkKind: 'reference'` の設問は `partitionQuestionsForSave` により既存 `questions/{id}` を参照 ID のみで `questionIds` に追加し、新規 `questions` ドキュメントは作成しない。 | `authorId: string`, `quizData: Omit<Quiz, 'id' \| 'authorId'>`, `isPublished: boolean` | `Promise<string>` (作成したクイズID) | 必要 |
| **`updateQuiz`** | クイズのメタ情報や問題配列を更新する。作成者がオリジナルタグを編集した場合は `originalTags` も更新し、それに基づいて `tags` および `canonicalTagIds` を再計算・更新する。**Phase 8**: 参照リンク設問の内容変更時は Copy-on-Write で `owned` 設問として新規保存。 | `id: string`, `authorId: string`, `updates: Partial<Quiz>`, `isPublished: boolean` | `Promise<void>` | 必要 |
...
| **`toggleBookmarkQuestion`** | 特定の設問をブックマーク登録/解除（トグル）。内部で `BookmarkService.toggleBookmark(..., 'question')` を呼ぶ。親クイズが `published` でない設問は登録不可。 | `userId: string`, `questionId: string` | `Promise<boolean>` | 必要 |
| **`getBookmarkedQuestions`** | ユーザー自身がブックマークした設問一覧（`Question[]`、登録日時降順）。 | `userId: string` | `Promise<Question[]>` | 必要 |
| **`addQuestionToList`** | 設問リスト（`listType === 'question'`）に設問を追加。親クイズが `published` であることを検証。クイズリストへの呼び出しは `ListTypeMismatchError`。 | `listId: string`, `questionId: string` | `Promise<void>` | 必要 |
| **`removeQuestionFromList`** | 設問リストから設問を削除。 | `listId: string`, `questionId: string` | `Promise<void>` | 必要 |

---

### 2.2b BookmarkService (`src/services/bookmark.ts`) [Phase 8]
クイズ・リスト・設問の分類ブックマークを統合管理します。

| メソッド名 | 説明 | 主要引数 | 戻り値 | 認証要否 |
| :--- | :--- | :--- | :--- | :--- |
| **`toggleBookmark`** | クイズ・リスト・設問のブックマーク登録/解除。設問は親クイズ公開ガードあり。 | `userId`, `targetId`, `targetType: 'quiz' \| 'list' \| 'question'` | `Promise<boolean>` | 必要 |
...
| **`enrichBookmarkedQuestions`** | ブックマーク設問を親クイズメタ付きで取得。**親が `published` の設問のみ**一覧に含める。 | `userId: string` | `Promise<BookmarkedQuestionEntry[]>` | 必要 |
...
クイズリスト／設問リストの CRUD、並び替え、エクスポートを担当します。
...
| **`getQuestionsInList`** | 設問リストの順序付きメンバー（親クイズタイトル付き）。 | `listId: string` | `Promise<QuestionInListEntry[]>` | 不要 |
| **`reorderQuestionList`** | 設問リストの `questionIds` 並び替え。 | `listId`, `newOrder: string[]` | `Promise<void>` | 必要 |
| **`exportQuestionList`** | 設問リスト JSON エクスポート（自作設問フル、他者設問は参照のみ）。 | `listId`, `authorId` | `Promise<QuestionListExportPackage>` | 必要 |
...
* **設問リストプレイ**: 各設問完了時に `Attempt.mode = 'question-list'`、`listId`、親 `quizId`、`totalQuestions: 1` で `saveAttempt`。UI 進行は `src/lib/question-list-session.ts`（sessionStorage）。
