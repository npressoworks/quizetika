...
| **`createQuiz`** | クイズを新規作成する（下書きまたは直接公開）。 | `authorId: string`, `quizData: Omit<Quiz, 'id' \| 'authorId'>`, `isPublished: boolean` | `Promise<string>` (作成したクイズID) | 必要 |
| **`updateQuiz`** | クイズのメタ情報や問題配列を更新する。 | `id: string`, `authorId: string`, `updates: Partial<Quiz>`, `isPublished: boolean` | `Promise<void>` | 必要 |
...
* **タグ・ジャンル仮想統合の検索クエリ自動展開仕様**:
  - `searchQuizzes` および `getQuizzesByTag` において、指定された `tag` または `genre` でクイズを検索する前に、サービスレイヤー（または Next.js サーバーサイド）は対応する `metadata_tags` / `metadata_genres` マスタドキュメントを1回フェッチします。
  - 取得したマスタの `mergedTagIds`（または `mergedGenreIds`）配列を元に、統合されたすべての旧タグID・ジャンルIDを含む「展開済み検索リスト」を構築します。
    ```typescript
    // 例: ユーザーが '#react' でタグ検索した場合
    const tagMaster = await db.collection('metadata_tags').doc('react').get();
    const mergedTagIds = tagMaster.data()?.mergedTagIds ?? [];
    const targetTags = ['react', ...mergedTagIds]; // ['react', 'reactjs', 'react-js']
    // Firestoreクエリに展開して発行
    query(collection(db, 'quizzes'), where('tags', 'array-contains-any', targetTags));
    ```
  - Firestoreの `array-contains-any` 演算子の上限（最大30要素）を意識し、マージ可能な子タグ数の上限は29件とします。
...
| **`askAiQuestion`** | 水平思考クイズ（ウミガメのスープ）において、AIへ自由記述の質問を送信し、判定結果と補足コメントを取得・履歴保存する。**①質問文字数上限**: `questionText` は最大100文字に制限し、超過時は `validation-error` をスロー。**②同一質問キャッシュ**: `aiQuestionsHistory` を照合し、`questionText` が完全一致する履歴が存在する場合はAI API呼び出しを行わず即座に既存の回答を返す（`isFromCache: true`）。**③ターン制限**: 無料ユーザーはセッション内の質問総数（`aiTurnCount`、キャッシュヒットはカウントしない）が20回に達した場合 `limit-exceeded` 例外をスロー。有料プランユーザー（`aiTurnLimit: null`）は無制限。**④AIステートレス一問一答**: AI API呼び出し時は会話履歴を送らず、「裏設定（`aiContextDetails`）＋ 1質問」のみをプロンプトに渡して判定する。**⑤セキュリティ**: APIキー漏洩防止のため、Next.js API Route または Cloud Functions 等のサーバーサイドを経由して呼び出し、クライアントからの直接呼び出しを禁止する。 | `attemptId: string`, `questionText: string` (max 100文字) | `Promise<{ answerType: 'yes' \| 'no' \| 'irrelevant' \| 'unknown', aiComment?: string, isFromCache: boolean }>` | 必要 |
...
| **`deleteUserAccount`** | ユーザーアカウントを削除し、関連データを匿名化して存続させる。Firebase Auth の物理削除、Firestore `users` ドキュメントの物理削除（または個人情報の削除・匿名化）、および自身が作成した公開 `quizzes`、`quizLists` や、過去に送信した `feedbackReports`、`notifications`、`reactions` に含まれる非正規化作成者情報を `"deleted_user"` および `"退会済みユーザー"` へとアトミックに書き換える。 | `uid: string` | `Promise<void>` | 必要 |
...
クイズ結果画面から行う「面白さ評価（星1〜5）」および「体感難易度投票」の永続化を担当します。

#### メソッド定義一覧

| メソッド名 | 説明 | 主要引数 | 戻り値 | 認証要否 |
| :--- | :--- | :--- | :--- | :--- |
| **`submitRating`** | 面白さ評価（星1〜5）および体感難易度投票をアトミックに保存する。`ratingAverage`・`ratingsCount` をトランザクションで更新。 | `quizId: string`, `userId: string \| null`, `starRating: number` (1-5), `difficultyVote: number` (1-10) | `Promise<void>` | 不要（匿名投票可） |
...
| メソッド名 | 説明 | 主要引数 | 戻り値 | 認証要否 |
| :--- | :--- | :--- | :--- | :--- |
| **`submitReview`** | クイズに対する良問/悪問評価を登録。全問解答済みのユーザーのみ可能。同一ユーザーが同一クイズを再度評価する場合は上書きとなる。自分のクイズは評価不可。**評価・変更・削除と同一トランザクション内で `quizzes/{quizId}` の `positiveCount` / `negativeCount` をリアルタイムにアトミック加減算更新する（週次バッチ処理の集計負荷削減）。** | `quizId: string`, `reviewerId: string`, `type: 'positive' \| 'negative'`, `reason?: string` | `Promise<void>` | 必要 |
| **`getReviewStats`** | 指定クイズの良問率・良問数・悪問数・バッジ指定を取得。 | `quizId: string` | `Promise<{ reviewScore: number \| null, positiveCount: number, negativeCount: number, reviewBadge: string \| null }>` | 不要 |
| **`submitReviewResetRequest`** | 「要改善」バッジのクイズの `reviewScore` リセット申請を `reviewResetRequests` コレクションに登録する。クイズの作成者のみ実行可能。 | `quizId: string`, `requesterId: string` | `Promise<string>` (リクエストID) | 必要 |
