...
並び替えクイズ (sorting) や連想クイズ (association) は、快適なUXを提供するためにフロントエンド（ブラウザ上）で即座に正誤判定を行いますが、プレイ結果を保存する際の不正な改ざん（チート行為）を物理的に排除するため、以下の強固な防衛策を適用します。
...
* **防衛策**: 
  ...
  - 各設問の問題タイプ（`sorting`, `association` 等）に応じて、サーバー側で回答の正誤を再シミュレーション検証します。
    - **並び替え**: クライアントから送信されたユーザーの最終並び替え順が、DBに登録された `correctOrder` と完全に合致しているかをサーバー側で再走査します。
