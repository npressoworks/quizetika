...
import { resetUserReports } from '@/services/reputation';

/**
 * 管理者用ユーザー通報数リセットAPI
 * POST /api/admin/users/reset-reports
 *
 * リクエストボディ:
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!reason || typeof reason !== 'string') {
      return NextResponse.json(
        { error: 'invalid-params', message: '通報数リセット理由(reason)は必須です。' },
        { status: 400 }
      );
    }
    ...
    if (reason.length < 10) {
      return NextResponse.json(
        { error: 'invalid-params', message: '通報数リセット理由は10文字以上で入力してください。' },
        { status: 400 }
      );
    }
    ...
    console.error('[API/admin/users/reset-reports] 予期しないエラー:', error);
