...
**Phase 26（2026-06-10）**: リスト機能完全廃止に伴い、本スペックの Phase 24 移行スコープから **リスト探索（`/lists`）・リスト詳細（`/list/[id]`）・`src/components/lists/*`** を除外する（要件 11 参照）。要件 5・6 および要件 9・10 のリスト関連部分は **廃止**。`quizeum-lists-discovery-ui` は **obsolete**。
...
**Phase 26（2026-06-10）**: `quizeum-core` Phase 26 によるリスト機能完全廃止に伴い、本スペックの Phase 24 移行スコープから **リスト探索（`/lists`）・リスト詳細（`/list/[id]`）・`src/components/lists/*`** を除外します。要件 5・6 および要件 9・10 のリスト関連部分は本フェーズで **廃止** します。ホーム・検索・ジャンル/タグ探索（`src/components/explore/*`）の shadcn 移行成果は維持します。
...
4. When [リスト関連ルート・コンポーネントの物理削除が必要なとき], the [Discovery UI] shall [実装責務を `quizeum-play-flow-ui` Phase 26 および `quizeum-core` Phase 26 に委譲すること]。
...
7. The [Discovery UI] shall [`quizeum-lists-discovery-ui` スペック全体を **obsolete** とし、リスト探索 UI の正本としないこと]。
