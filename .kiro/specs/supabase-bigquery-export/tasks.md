...
- [x] 3.3 (P) BigQuery insertAllクライアントの実装
...
- [ ] 4. テスト
- [x] 4.1 (P) Edge Function純ロジックのJest単体テスト
...
- [x] 4.2 (P) キャプチャ層の統合テスト(ローカルSupabase)
...
- 実装完了後の運用メモ(2026-07-11): BigQueryターゲットプロジェクトをユーザーの指示で`quizeum-77bc6`から新規GCPプロジェクト`quizetika`へ切り替えた。`quizetika`にIAM API有効化、サービスアカウント`bigquery-export-sa@quizetika.iam.gserviceaccount.com`(`roles/bigquery.dataEditor`のみ)を作成し、`scripts/bigquery/setup.sql`・`views.sql`を適用して`raw_events`テーブル+分析ビュー10本を作成・検証済み。`scripts/bigquery/README.md`を`quizetika`基準に更新し、旧`quizeum-77bc6`側の経緯を移行注記として残した。旧`quizeum-77bc6`側のリソース(SA・データセット・テーブル・ビュー)は削除しておらずそのまま残存している(要ユーザー判断)。Supabase Edge Functionの本番デプロイ・secrets設定・Vault実値更新はまだ実施していない(READMEのセットアップ手順3・4が未完了)。
- 本番デプロイ完了記録(2026-07-11続き): Supabaseプロジェクト`quizetika`(project-ref: `zlghfzitgdqoodssvspu`)は既にリンク済み・全マイグレーション適用済み・Edge Function `bigquery-export`もデプロイ済みだった(本セッション開始前の別作業と思われる)。secrets 4項目とVault実値を設定し、本番`analytics_outbox`へテスト行を投入する実E2E検証を実施。**重大な発見**: `supabase secrets set KEY="$(Get-Content ...)"`のようにJSON文字列をインライン引数で渡すと、PowerShell→npx→Nodeの引数受け渡し過程で**JSON内の二重引用符がサイレントに失われ**(`{id: x}`のような壊れたJSONになる)、Edge Function側で`JSON.parse()`失敗→`{"error":"internal error"}`(HTTP 500)が返る分かりにくい障害になることが判明(配送トリガー自体は200 OKでリクエスト到達するため、`net._http_response`のstatus_codeを見ないと気づけない)。`--env-file`オプションでファイル経由に切り替えて解決した。この教訓をREADMEの「シークレット設定」セクションに反映済み。修正後、本番`analytics_outbox`→Edge Function→BigQuery `quizetika.quizetika_analytics.raw_events`への実到達を確認し、テスト行をクリーンアップ(outbox側は削除済み、BigQuery側の2件はstreaming buffer制約で当面削除不可・無害な残存として許容)。
...
- タスク4.2: `supabase/tests/bigquery-export-capture-triggers.sql`はpgTAP未導入のためプレーンSQL+`RAISE EXCEPTION`方式を採用し、`BEGIN...ROLLBACK`で自己クリーニング(再実行可能)。実`handle_save_attempt` RPCを呼び出して4シナリオ(question_answer_details含むattempts INSERT、quizzes/questions UPDATEでのPII除外、users UPDATEでのoutbox無反応、attempts DELETEでのOLDスナップショット)を検証。実行方法: `Get-Content -Raw -Encoding UTF8 supabase\tests\bigquery-export-capture-triggers.sql | docker exec -i supabase_db_quizetika psql -U postgres -d postgres -v ON_ERROR_STOP=1`(ホスト→Dockerポートフォワーディングが機能しない環境向け)。
- タスク4.1: 検証のみ(コード変更なし)。task 3.2/3.3/3.5がTDDで既に4つの受け入れ基準(insertId=event_id、部分失敗分類、JWT組立+キャッシュ、シークレット不一致401)を全てカバーしていたため追加実装は不要だった。design.mdは単一ファイル`tests/functions/bigquery-export.test.ts`を想定していたが、実際はモジュール別に`tests/functions/bigquery-export/{google-auth,bigquery,outbox,index}.test.ts`に分割されており、これは意図的な改善として妥当と判断。
- タスク3.5: 実E2E検証(ユーザー承認によりGCPサービスアカウント`bigquery-export-sa@quizeum-77bc6.iam.gserviceaccount.com`(`roles/bigquery.dataEditor`のみ)を新規発行して実施)で、task 3.3の`bigquery.ts`に実バグを発見・修正した。BigQuery `tabledata.insertAll`(streaming insert)はJSON型カラムの値をネストしたオブジェクトではなくJSON文字列として要求する(`"payload is not a record"`エラー)。`payload: event.payload`→`payload: JSON.stringify(event.payload)`に修正し、回帰防止テストを追加済み(Jestのfetchモックでは検出できなかった箇所なので今後同様のBigQuery API統合を追加する際は要注意)。また、BigQuery `raw_events`にテスト用行2件(`event_id`が`11111111-...`と`22222222-...`)が残存している。streaming insert bufferの制約で挿入後最大90分程度DELETEできないため未クリーンアップ。後日`bq query`で削除するか、無害なテストデータとして放置してよい。GCP SA鍵ファイルはスクラッチパッドに残置(リポジトリには一切含まれない)。加えて、`index.ts`はgoogle-auth.ts/bigquery.ts/outbox.tsを値としてimportする初めてのファイルであり、リポジトリルートの`tsconfig.json`(Next.js向け、`moduleResolution: "bundler"`)では`TS5097`(相対import拡張子)と`Deno`グローバル未定義のtsc警告が出る。これはDenoターゲットコードを Next.js向けtsconfigで検査した際の既知の構造的差異であり、実行時の不具合ではない(`deno check`が使えればそちらが正しい検証手段だが本環境にはDeno CLI未インストール)。将来的に`supabase/functions/`専用のtsconfigを用意する改善余地がある。
