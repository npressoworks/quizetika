# Brief: quizetika-creator-dash-ui

## Problem
クリエイターがクイズやクイズリストを作成・編集し、投稿したクイズのアナリティクス分析やプレイヤーからの間違い指摘フィードバックを確認・処理する、高品質なクリエイターUIが必要です。

## Current State
クイズエディタ・クリエイターダッシュボード・リスト編集 UI は実装済み。Phase 6 では `QuizEditor` のジャンル `<select>` がハードコード 6 件のまま（`listActiveGenres` 未接続）。`quizetika-core` Phase 6 完了後、play-flow と同様にマスタ駆動セレクトへ差し替える。
...
## Desired Outcome
直感的で使いやすいクイズ・問題エディタ（動的な追加削除、自動名寄せサジェスト警告、公開時Zodバリデーション警告など）を提供し、クリエイターダッシュボードで円グラフ等のアナリティクスや指摘管理を快適に行え、クイズリストのドラッグ＆ドロップ編集やJSONパッケージエクスポートを行えること。
...
- **In**:
  ...
  - クリエイターダッシュボード (`/creator/dashboard`): 累計プレイ数、評価平均、ブックマーク数のグラフ表示。各クイズの問題別正解率・解答割合のビジュアルグラフ表示。クローズド指摘（誤植・事実誤認等）リストと修正動線。クイズ一括JSONエクスポート。
...
## Upstream / Downstream
- **Upstream**: `quizetika-play-flow-ui`, `quizetika-core`
- **Downstream**: `quizetika-moderation-governance-ui`

## Existing Spec Touchpoints
- **Extends**: `quizetika-core` の `QuizService`, `QuizListService` および関連データモデル。
- **Adjacent**: `quizetika-play-flow-ui` (クイズ詳細からのクリエイターダッシュボード遷移など)
