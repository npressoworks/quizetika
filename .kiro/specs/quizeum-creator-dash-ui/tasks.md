...
- [x] 8.2 エディタテストコードの修正
  - 作問エディタおよびクリエイターダッシュボードに関連するテストにおいて、難易度が 6 以上の入力や期待値になっている箇所を 1〜5 の範囲に修正する。
  - **完了状態**: ダッシュボードおよびエディタ関連の Jest テストがすべて正常にパスすること。
  - _Requirements: 1.1_
  - _Boundary: Testing_

- [x] 8.3 Phase 13 クリエイター UI 統合検証
  - 作問エディタで難易度 1〜5 を設定して下書き保存および公開保存ができ、クリエイターダッシュボードでも正しく動作することを確認する。
...
- [x] 9.1 クリエイターダッシュボードの Server Component 化と Suspense 導入 (P)
...
- [x] 9.4 認証必須画面における Middleware サーバーサイド認証保護の実装
  - クリエイターダッシュボード（`/creator/dashboard`）やクイズ編集画面（`/quiz/[id]/edit`）について、未ログインアクセス時にクライアント側リダイレクトによる白紙表示を防ぐため、`src/middleware.ts` を作成（または更新）し、サーバーサイドで即時リダイレクト（`307`）制御する。
...
- [x] 10.2 出題形式カードと複合形式トグルの拡張
  - クイズ全体の出題形式選択に「〇✕問題」を追加し、選択時は全問題を `true-false` に固定して問題タイプ切り替え UI を非表示にする
  - 複合形式の問題タイプトグルに「〇×」を追加し、`data-testid="question-type-true-false"` を付与する
  - **完了状態**: 〇✕問題形式選択と複合トグルから `true-false` 問題を作成・切り替えできること
  - _Requirements: 11.1, 11.2, 11.4, 11.5, 11.14_
  - _Depends: 10.1, quizetika-core 19.2_
  - _Boundary: QuizEditor_

- [x] 10.3 形式変換とデフォルト問題追加の整合
  - 他形式から「〇✕問題」への一括変換時に確認ダイアログを表示し、同意後に各問題へ固定選択肢とデフォルト正解を設定する
...
- [x] 10.5 Phase 20 統合検証
  - 〇✕問題クイズの下書き保存がコア検証を通過することを確認する
...
- [x] 11.1 クリエイターダッシュボードからリスト作成導線の除去
...
- [x] 11.3 Phase 26 統合検証
  ...
  - クリエイターダッシュボードの Suspense スケルトン（`quiz-list-skeleton` 等・クイズ一覧用）が維持されることを確認する
...
### 12. Phase 27: クリエイター＆プレイヤー統合ダッシュボード（2026-06-28）
...
- [x] 12.4 ダッシュボード画面へのタブ切り替え統合
  ...
  - `src/app/creator/dashboard/dashboard-client.tsx` に `Tabs` を導入し、デフォルトで「プレイヤーダッシュボード」、もう一方に「クリエイターダッシュボード」を配置する。
  - **完了状態**: URL `/creator/dashboard` アクセス時に「プレイヤー」「クリエイター」のタブが表示され、デフォルトでプレイヤーダッシュボードが表示されること。
  - _Requirements: 13.1_
...
### 14. Phase 40: 作成したクイズ画面（2026-07-12）
...
- [x] 14.4 作成したクイズ画面のデータ取得・状態管理の実装
...
- [x] 14.5 (P) 作成したクイズ画面の一覧・フィルタ表示UIの実装
...
- [x] 14.8 クリエイターダッシュボードの簡易クイズ一覧を管理画面導線へ置換
  - クリエイターダッシュボードの簡易クイズ一覧セクション（検索・フィルタなしの一覧表示）を撤去し、作成したクイズ画面への導線カードに置き換える。
  - ダッシュボードのアナリティクス・グラフ・プレイヤー統計表示ロジックには変更を加えない。
  - **完了状態**: ダッシュボードに簡易クイズ一覧が表示されなくなり、導線カードのクリックで作成したクイズ画面へ遷移すること。
  - _Requirements: 19.1, 19.2, 19.3, 19.4_
  - _Boundary: CreatorDashboard_
  - _Depends: 14.7_

- [x] 14.9 (P) 統合ステータス・検索/絞り込み/並び替えロジックの単体テスト
  ...
  - _Depends: 14.1, 14.2_
  - **実施記録**: 本タスクが求める観点は、タスク14.1・14.2のTDD実装時に `tests/lib/creator-quiz-status.test.ts`（7件）・`tests/services/author-quiz-search.test.ts`（11件）として既に作成・レビュー承認済み。重複実装を避け、`npx jest tests/lib/creator-quiz-status.test.ts tests/services/author-quiz-search.test.ts` で全件成功（フレッシュ実行で確認済み）を根拠に完了とする。

- [x] 14.10 (P) 指摘件数集計機能の単体テスト
  ...
  - _Depends: 14.3_
  - **実施記録**: 本タスクが求める観点は、タスク14.3のTDD実装時に `tests/services/review.test.ts` の `getOpenReportCountsByCreator` 用テスト（複数クイズ集計・0件除外・空結果・エラー時フォールバックの4件）として既に作成・レビュー承認済み。重複実装を避け、フレッシュ実行での全件成功を根拠に完了とする。

- [x] 14.11 公開範囲切り替えUIの結合テスト
  ...
  - _Depends: 14.6_
  - **実施記録**: 本タスクが求める観点は、タスク14.6のTDD実装時に `tests/components/creator-quiz-visibility-toggle.test.tsx`（`updateQuiz` をモックした6件: 無料プランでの非活性表示、Pro必須エラー時の表示・復帰、汎用エラー時の表示・復帰 等）として既に作成・レビュー承認済み。重複実装を避け、フレッシュ実行で6件全成功を確認済み。

- [x] 14.12 Phase 40 E2Eテストの作成・更新
  - 新規E2Eスモークテストとして、作成したクイズ画面での一覧表示→キーワード検索→統合ステータス/ジャンル/タグ絞り込み→並び替え→新規作成導線→編集導線→指摘バッジ→編集画面遷移、の一連のフローを検証する。
  - 下書き・審査により非表示（凍結）ステータスの行に公開範囲切り替えUIが表示されないことを検証する。
  - 既存のクリエイターダッシュボードE2Eテストを更新し、簡易クイズ一覧が表示されないこと、管理画面への導線が機能することを検証する。
  - **タスク14.8のレビューで判明した波及修正（必須）**: `creator-quiz-list`（および `quiz-card`）testidは14.8で撤去済みのため、これに依存している以下の既存E2Eも本タスクで併せて更新する。いずれも「ダッシュボードでクイズ一覧を確認する」箇所を `/creator/quizzes` への遷移＋同ページでの確認に置き換える（`e2e/creator-dashboard.spec.ts` 以外の3ファイルは新規追加ではなく既存フローの導線変更に伴う修正）。
    - `e2e/creator-streaming-skeleton.spec.ts`（19行目 `creator-quiz-list` の可視性チェック）
    - `e2e/quiz-creation.spec.ts`（76行目、下書き作成後にダッシュボードで確認する箇所）
    - `e2e/seo-sharing.spec.ts`（291行目、作成したクイズの編集ボタンからクイズIDを取得する箇所）
    - `e2e/additional-features.spec.ts`（379行目、作成したクイズの確認箇所）
  - **完了状態**: 新規・更新したE2Eテスト、および上記4ファイルの既存E2Eテストがすべて成功すること。
  - _Requirements: 15.1, 15.2, 15.4, 15.6, 16.7, 16.8, 17.2, 17.3, 17.9, 18.3, 19.1, 19.3_
  - _Boundary: Testing (E2E)_
  - _Depends: 14.7, 14.8_

- [x] 14.13 Phase 40 統合検証
  - 作成したクイズ画面とクリエイターダッシュボードの結線を通しで確認し、既存機能（クイズ作成・編集・アナリティクス・指摘解決サイドバー）に回帰がないことを確認する。
...
## Phase 44: リッチダッシュボード刷新 — フィルタ・ドリルダウン UI

- [ ] 17. フィルタ基盤とプレイヤータブ刷新
- [x] 17.1 フィルタ状態管理とフィルタバー
  - タブごとに独立したフィルタ状態（期間の初期値は直近30日、他は「すべて」）を管理するフックを実装する
  - 両タブに軸構成の異なるフィルタバー（`data-testid="dashboard-filter-bar"`）を表示し、適用中条件の表示と一括リセット操作を提供する
  - 完了条件: タブ切り替えで相互のフィルタ状態が影響し合わないことと初期値・リセット挙動がユニットテストで検証される
  - _Requirements: 21.1, 21.2, 21.3, 21.6, 21.7_
  - _Boundary: DashboardFilterBar / useDashboardFilters_

- [x] 17.2 プレイヤー統計のサーバー集計移行
  - 直近100件のクライアント集計（履歴取得 + クイズ N+1 取得）を廃止し、フィルタ連動のコア集計呼び出しへ置き換える
  - フィルタ変更時はページ遷移なしで再取得し、取得中はセクションスケルトンを表示、リクエスト連番ガードで旧結果との混在を防ぐ
  - 取得失敗時はエラーカードと再試行ボタン、母集団0件時は空状態を表示する
  - 完了条件: フィルタ変更で新しいフィルタ引数のサービス呼び出しが発生し、ローディング→結果／エラー→再試行がコンポーネントテストで検証される
  - _Requirements: 21.4, 21.5, 21.8, 22.1, 22.9_
  - _Boundary: player-dashboard-client_
  - _Depends: 17.1_

- [x] 17.3 プレイヤーセクションの表示拡張
  - 基本統計カードに合計プレイ時間・連続プレイ日数を追加する
  - 期間粒度連動のトレンドチャート（複数系列対応の共有チャートコンポーネントを新設して使用）と設問形式別内訳セクション（`data-testid="player-format-analysis"`）を追加する
  - 苦手分析（正答率下位、母数3件以上）と該当ジャンル・タグの探索導線を表示する
  - ワードクラウドの母集団をフィルタ適用後集計（タグ集計 + タイトル集計からのキーワード展開）に連動させる
  - 完了条件: 既存 `data-testid`（`player-stats` / `player-charts` / `player-genre-tag-analysis` / `player-word-cloud`）が維持されたまま新セクションが表示され、コンポーネントテストが通る
  - _Requirements: 22.2, 22.3, 22.4, 22.5, 22.6, 22.7, 22.8_
  - _Boundary: dashboard-sections / TrendChart / word-cloud_
  - _Depends: 17.2_

- [x] 17.4 プレイヤードリルダウン
  - 内訳・分析セクションの項目選択から、適用中フィルタと合成した該当プレイ履歴一覧（`data-testid="player-drilldown"`、もっと見るページング）を表示する
  - 履歴エントリ選択で設問別の正誤・解答内容・解答時間・ヒント使用数の明細を表示し、明細のない旧試行は要約と注記を表示する
  - ドリルダウン中もフィルタバーを有効とし、変更時は一覧を新条件で再取得（明細表示中は一覧へ戻す）、概要表示へ戻る導線を提供する
  - 完了条件: 項目選択→一覧→明細→戻るの一連の遷移と明細なし注記がコンポーネントテストで検証される
  - _Requirements: 23.1, 23.2, 23.3, 23.4, 23.5, 21.4_
  - _Boundary: player-drilldown_
  - _Depends: 17.2_

- [x] 18. クリエイタータブ刷新
- [x] 18.1 (P) クリエイター集計移行と KPI・トレンド
  - キャッシュカウンタ合算のクライアント集計をフィルタ連動のコア集計呼び出しへ置き換え、期間 KPI（プレイ・ユニークプレイヤー・ブックマーク・評価・完走率）とトレンドチャートを表示する
  - 完走率のサンプル不足時は「データ蓄積中」注記を表示する
  - モック指摘注入を撤去し、指摘0件時は空状態カードを表示する（指摘キューの既存機能は維持）
  - 完了条件: 指摘0件でモックが表示されず空状態になること、蓄積中注記、フィルタ連動の再取得がコンポーネントテストで検証される
  - _Requirements: 24.1, 24.2, 24.6, 24.7, 24.8, 24.9_
  - _Boundary: dashboard-client（クリエイター）/ creator-dashboard-sections_
  - _Depends: 17.1_

- [x] 18.2 クイズ別ランキングと設問形式別パフォーマンス
  - 期間内プレイ数・平均正答率・ブックマーク・評価で並び替え可能なクイズ別ランキング（`data-testid="creator-quiz-ranking"`）を表示する
  - 設問形式別の解答数・正答率セクションを表示する
  - 完了条件: 各指標ヘッダの操作で並び順が切り替わることがコンポーネントテストで検証される
  - _Requirements: 24.3, 24.5_
  - _Boundary: creator-dashboard-sections_
  - _Depends: 18.1_

- [x] 18.3 クイズ単体分析ビュー
  - ランキング行の選択でクイズ単体分析ビュー（`data-testid="creator-quiz-analysis"`）へ遷移し、設問別累計正答率・選択肢別解答分布（「累計値・期間フィルタ対象外」の明示付き）、期間連動スコア分布、離脱分布と完走率を表示する
  - 離脱データ不足時の蓄積中注記、分析取得失敗時のエラー/空状態、クリエイターダッシュボードへ戻る導線を提供する
  - フィルタ変更時は分析ビューを新条件で再取得する
  - 完了条件: ランキング行選択→分析表示→戻るの遷移と累計ラベル・注記・エラー状態がコンポーネントテストで検証される
  - _Requirements: 24.4, 25.1, 25.2, 25.3, 25.4, 25.5, 25.6, 25.7, 21.4_
  - _Boundary: creator-quiz-analysis_
  - _Depends: 18.2_

- [x] 19. 統合検証
- [x] 19.1 Jest 回帰更新と全体検証
  - 既存ダッシュボード系テストのサービスモックを新しいダッシュボードサービスベースへ差し替え、モック指摘前提の既存 assert を空状態 assert に置換する
  - 完了条件: `npm run build`・`npm run test` が成功し、ダッシュボード関連スイートがすべてグリーンである
  - _Requirements: 22.8, 24.8_
  - _Depends: 17.4, 18.3_

- [x] 19.2 E2E 更新
  - ローカル Supabase への実データ投入（解答明細付き試行・ブックマーク・評価）を前提に、フィルタ変更でのセクション更新・形式別セクション・ドリルダウン往復・ランキング並び替え・クイズ単体分析往復を検証する
  - 既存 testid（`player-stats` 等）の回帰 assert を維持し、データ投入が困難なセクションは空状態検証まで縮退可とする
  - 完了条件: `e2e/creator-dashboard.spec.ts` がローカルでグリーンであり、`npm run e2e:diff` で新規デグレードがない
  - _Requirements: 21.1, 22.8, 23.1, 24.3, 25.1_
  - _Depends: 19.1_

## Implementation Notes (Phase 44)

- **前提**: `quizetika-core` Phase 44 のタスク 32.x（サービス層契約）完了後に着手すること（本フェーズの全タスクが `@/services/dashboard` / `@/types/dashboard` に依存）。
- `src/lib/player-stats.ts` の改変は core が単独所有。本スペックは新契約の import 追随のみを行う（design.md レビュー結果の反映参照）。
- 18.1 は 17.2〜17.4 と異なるファイル群（クリエイター側）を触るため並行可（17.1 完了が前提）。
- 共有チャートコンポーネント（TrendChart）は 17.3 で新設し、18.1 はそれを再利用する。18.1 を 17.3 より先に実施する場合は TrendChart の新設を 18.1 側へ移す。
