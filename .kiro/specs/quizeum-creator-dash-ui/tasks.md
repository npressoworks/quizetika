...
- [x] 14.8 作家ダッシュボードの簡易クイズ一覧を管理画面導線へ置換
  - 作家ダッシュボードの簡易クイズ一覧セクション（検索・フィルタなしの一覧表示）を撤去し、作成したクイズ画面への導線カードに置き換える。
...
- [x] 14.13 Phase 40 統合検証
  - 作成したクイズ画面と作家ダッシュボードの結線を通しで確認し、既存機能（クイズ作成・編集・アナリティクス・指摘解決サイドバー）に回帰がないことを確認する。
