...
**Phase 12（2026-06）**: 作問エディタの主要テキストエリア（説明文・問題文・真相・解説文）の自動伸長、過去自作クイズ検索の設問文・正解テキスト対応、参照リンク操作時の成功フィードバック、設問文ヒット時のアコーディオン自動展開、リンク済み設問の「リンク解除」、およびテストプレイ後の編集画面復帰時における設問数の不変保持を追加する（設問文・正解テキスト照合ロジックは `quizeum-core` が担当）。
...
- **対象範囲（In scope）**:
  ...
  - **Phase 12**: 作問エディタの説明文・問題文・真相・解説文テキストエリアの自動伸長、過去自作クイズ検索パネルの設問文・正解テキスト検索、参照リンク成功メッセージ、設問文ヒット時のアコーディオン自動展開、リンク済み設問のリンク解除、テストプレイ後の編集画面ドラフト復元。
- **対象外（Out of scope）**:
...
- **隣接システムへの期待（Phase 12）**:
  - 過去自作クイズ検索における設問文・正解テキストの照合ロジックは `quizeum-core`（`searchAuthorQuizzes` 拡張）が担当。本スペックは検索 UI・結果一覧・リンク操作フィードバックを担当する。
  - テストプレイの解答判定・結果保存・統計非記録は `src/lib/test-play.ts` およびテストプレイ画面が担当。本スペックはエディタ状態の `sessionStorage` 保存と復帰時の設問数保持を担当する。
...
11. When 作成者が過去自作クイズ検索パネルでキーワードを入力したとき、クイズエディタ画面は、クイズのタイトルおよび説明文に加え、各設問の問題文および設問タイプに応じた正解テキスト（選択式・正誤式の正解選択肢文、記述式・即答式・連想式の正解候補、並び替え式の各要素文、ウミガメのスープの必須正解キーワード）でも一致する自作クイズを検索結果に含めること。
12. The クイズエディタ画面は、過去自作クイズ検索パネルのキーワード入力欄に、タイトル・説明・問題文・正解テキストが検索対象であることを示すプレースホルダーまたは説明文を表示すること。
13. When 作成者が過去自作クイズ検索パネルで設問のリンク操作を成功したとき、クイズエディタ画面は、リンクが完了したことを作成者が認識できる成功メッセージを表示すること。
14. If 作成者が既にリンク済みの設問を検索パネルから操作するとき、クイズエディタ画面は、「リンク」ボタンの代わりに「リンク解除」ボタンを表示し、リンク解除によりエディタ上の当該設問のみを削除すること（要件 7.8）。重複「リンク」操作は防止すること。
15. When 作成者が過去自作クイズ検索パネルでキーワードを入力し、いずれかの設問の問題文がキーワードに一致したとき、クイズエディタ画面は、当該クイズのアコーディオンを自動展開し、一致する設問を即時表示すること。問題文ヒットの設問は正解テキストのみヒットの設問より先頭に表示してよい。
16. When 作成者がリンク済み設問の「リンク解除」を実行したとき、クイズエディタ画面は、リンク解除が完了したことを作成者が認識できるフィードバック（成功メッセージ等）を表示してよい。
...
#### 受け入れ基準
1. The クイズエディタ画面は、クイズ全体の説明文、各設問の問題文、ウミガメのスープ設問の真相入力、各設問の解説文のテキストエリアを表示すること。
...
#### 受け入れ基準
1. 作成者がクイズエディタからテストプレイを開始したとき、クイズエディタ画面は、未保存のエディタ状態（設問配列を含む）を `sessionStorage` に保存し、テストプレイ画面へ遷移すること。
...
4. When テストプレイドラフトの復元に成功したとき、クイズエディタ画面は、復元した設問数を変更せず表示し、Firestore からの通常ロード（`getQuiz`）および新規作成時のデフォルト設問追加を実行してはならない。
5. When テストプレイドラフトの復元が完了したとき、クイズエディタ画面は、`fromTestPlay` クエリを除去した URL へ置換遷移し、同一セッション内の再マウントで設問が重複しないこと。
6. If `fromTestPlay=1` が付与されているが認証ユーザーが未確定のとき、クイズエディタ画面は、通常ロード（`getQuiz` / デフォルト設問追加）を開始してはならない。
7. If テストプレイ payload が欠落または期限切れのとき、クイズエディタ画面は、クエリを除去して復帰し、設問の重複追加を行ってはならない（編集モードでは以降の通常ロードに委ねてよい）。
