...
## Introduction
Quizetika ユーザーは表示テーマ（ダーク / ライト）などのアプリ設定を変更したいが、現状は Sidebar フッターのアカウントポップアップに「プロフィール」「ログアウト」のみで、**設定画面もテーマ切替も存在しない**。アプリは `variables.css` のダークトークン固定で、ライトモード用の定義はプレースホルダー程度であり、`layout.tsx` にテーマ Provider もない。
...
- **Adjacent expectations**:
  - `quizetika-sidebar-layout` が Sidebar フッターアカウントポップアップに「設定」リンク（`/settings`）を追加し、既存「プロフィール」「ログアウト」と同型の `popupItem` パターンで配置すること
