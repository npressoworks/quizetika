...
- [ ] 2.1 (P) PC/タブレット用サイドバーの作成とレスポンシブスタイリング
  ...
  - *検証結果*: ブラウザの画面幅を伸縮させたとき、PC版（ラベルあり）とタブレット版（アイコンのみ）がブレークポイント（1024px, 768px）で正しく伸縮し、スマホサイズでは非表示になること。
  - _Requirements: 1.1, 1.2_
  - _Boundary: Sidebar_

- [ ] 2.2 (P) サイドバーのメニュー動的表示とアバターポップアップの実装
...
## 4. Validation: 検証とテスト
- [ ] 4.1 Playwrightによるレイアウト＆ナビゲーションのE2Eテストの実装
...
- [ ]* 4.2 コンポーネント描画および状態出し分けの単体テストの追加
