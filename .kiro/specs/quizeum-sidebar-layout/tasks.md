...
- [x] 2.4 (P) モバイル専用ミニヘッダーへの軽量化
  ...
  - 767px以下でのみ表示（768px以上は非表示）し、左端にロゴ、右端に「クイズを作る（アイコン）」および「ユーザーアバター」を配置する。未ログイン時はログインリンクを表示する。
...
- [x] 8.1 (P) Sidebar への管理者メニュー主要ナビ導線追加
  ...
  - `user` が存在し、かつ `isAdminUser(user)` が真の場合に、「ダッシュボード」の下、「クイズを作る」ボタンの上に「管理者メニュー」リンク（遷移先: `/admin`）を追加する。
