...
**Phase 20（2026-06-09）**: 〇✕問題（`true-false`）に専用の `TrueFalseAnswerPanel` を導入し、〇／× ボタン1タップで即回答。本番プレイ・テストプレイ・弱点克服に適用。形式ラベル「〇✕問題」と探索カルーセルへの追加を含む（core / creator-dash がデータ・作問を担当）。
...
| 5.4          | クリエイターフォローボタン表示およびトグル制御                                                  | `/quiz/[id]/result` Page                                                            | `UserService`                                    | -                                          |
...
| 9.5          | **（Phase 38 改定）** 初回プレイの永続化記録のみ表示（フォールバック廃止）                      | `QuizDualLeaderboard`                                                               | `getLeaderboard(quizId, 'first_play')`           | -                                          |
| 9.6          | **（Phase 38 改定）** リプレイの永続化記録のみ表示                                              | `QuizDualLeaderboard`                                                               | `getLeaderboard(quizId, 'replay')`               | -                                          |
...
- **オフライン時の制限処理**:
  - オフラインでのプレイ中、結果画面に遷移した際は、「現在オフラインのため、良問評価や間違い指摘、クリエイターリアクションは送信できません。オンライン復帰後に自動同期されます。」と優しいトーンで警告ヘッダーを表示し、関連ボタンを非活性化（disabled）します。
...
| 形式ラベル「〇✕問題」・探索カルーセル  | 採点ロジック変更（既存 `isChoiceAnswerCorrect`） |
