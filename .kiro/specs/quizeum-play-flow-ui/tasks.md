...
- [x] 5.2 評価・難易度投票、間違い指摘、クリエイターリアクションUIの実装
  - 👍/👎良問評価ボタン、1-10の難易度投票、間違い・別解指摘フォームモーダル、クリエイターへのお礼リアクション送信ボタンを実装する。
...
### 18. Phase 14 拡張 — クリエイターお礼機能の廃止および結果画面でのフォロー機能の追加（2026-06）

- [x] 18.1 クリエイターお礼リアクションボタンの削除とフォローボタンの配置 (P)
...
- [x] 25.4 出題形式ラベルと探索カルーセルの拡張
  - クイズカード等の出題形式表示で `true-false` を「〇✕問題」として表示する（コア lib のラベル定義を利用）
  - ホームの出題形式カルーセルに「〇✕問題」を追加し、選択時に形式フィルタが `true-false` になること
  - **完了状態**: カード・カルーセルに「〇✕問題」が表示され、カルーセル選択でグリッドが絞り込まれること
...
### 29. Phase 37: クイズ詳細画面のプレイモード選択廃止・既プレイ限定の代替導線（2026-07-07）

- [x] 29.1 単一プレイボタン化（3択UI廃止）
  - クイズ詳細画面の `selectedMode` state と、通常形式向けの通常・模擬試験・フラッシュカードの3択プレイパネル（3分岐）のみを削除する
  - 通常形式のクイズでは、常に単一の「プレイ」ボタンのみを表示し、押下時は常に通常モードのプレイ画面へ遷移するようにする
  - 水平思考（ウミガメのスープ）・早押し形式向けの既存の単一モード表示・遷移先・マークアップ・スタイルクラスは変更しない（これらの分岐は3択UIとは別の分岐であり、共有スタイルクラスは維持する）
  - プレイパネルの見出しを「プレイモード選択」から「プレイ」に変更する（全分岐で共通表示のため）
  - **完了状態**: 未プレイの新規クイズ詳細画面を表示すると、モード選択UIは一切なく「プレイ」という見出しの下に「プレイ」ボタン1つのみが表示され、押下すると通常モードのプレイ画面へ遷移すること。水平思考・早押し形式のクイズの表示・スタイルに変化がないこと
  - _Requirements: 2.3, 2.3a, 2.4_
  - _Boundary: QuizDetailClient_

- [x] 29.2 既プレイ限定の代替導線とランキング非対象警告の表示条件反転・文言修正
  - 認証済みユーザーが当該クイズを完了済みと判定される場合（既存のプレイ済み判定を再利用）にのみ、単一プレイボタンの下に模擬試験・フラッシュカードへの代替導線を追加表示する新規スタイルクラスを追加し、マークアップを実装する
  - 代替導線から模擬試験・フラッシュカードそれぞれのプレイ画面へ正しく遷移できるようにする
  - ランキング非対象警告の表示条件を、現行の「未プレイ時のみ表示」から「代替導線が表示されるとき（＝既プレイ時）のみ表示」へ反転する
  - 警告文言から「先にこれらのモードでプレイした場合、以降通常モードでも初回プレイランキングに掲載されない」というプレイ順序依存の一文を除去し、「プレイ回数や順序にかかわらずランキングに影響しない」という順序非依存の表現に修正する
  - 水平思考・早押し形式では、完了状況にかかわらず代替導線・警告のいずれも表示しない
  - 未認証ユーザーには代替導線を表示しない
  - **完了状態**: 通常モードを1周完了したユーザーが同じクイズの詳細画面に戻ると、代替導線とランキング非対象警告（順序非依存の新文言）が表示され、模擬試験・フラッシュカードそれぞれへ正しく遷移できること。未プレイのクイズでは両方とも非表示のままであること
  - _Requirements: 2.3b, 19.1, 19.2, 19.3, 19.4, 19.5, 19.6, 19.9, 27.1, 27.2, 27.3, 27.4, 27.5, 27.8, 27.9_
  - _Depends: 29.1_
  - _Boundary: QuizDetailClient_

- [x] 29.3 (P) コンポーネントテストの更新（既存テストの反転修正含む）
  - 既存の「プレイ済みクイズでは警告を表示しない」テストを、既プレイ時に警告と代替導線が表示されることを検証するテストへ反転修正する
  - 既存の「通常クイズで警告を表示する」テストを、未プレイのクイズでは警告・代替導線ともに表示されないことを検証するテストへ更新する
  - 単一プレイボタンのみが表示されること、および代替導線から模擬試験・フラッシュカードへの遷移が正しく行われることを検証するテストを追加する
  - 水平思考・早押し形式のクイズでは完了状況にかかわらず代替導線・警告が表示されないことを検証するテストを維持する
  - **完了状態**: `tests/components/quiz-detail-client.test.tsx` の全テストが新しい表示条件と整合してグリーンであること
  - _Requirements: 2.3, 2.3a, 2.3b, 19.1, 19.2, 19.3, 19.4, 19.5, 19.6, 19.9, 27.1, 27.2, 27.3, 27.4, 27.5, 27.9_
  - _Depends: 29.2_
  - _Boundary: Testing_

- [x] 29.4 (P) E2Eテストの更新（既存3ファイル）
  - `e2e/learning-support.spec.ts` の模擬試験・フラッシュカードE2Eを、初回に通常モードで1周完了させたうえで詳細画面に戻り、代替導線からモード選択する手順に更新する
  - `e2e/quiz-play.spec.ts` の、新規クイズ初回訪問時にランキング非対象警告が常時表示されることを前提としたテストを、未プレイ時は警告非表示・単一「プレイ」ボタンのみであることを検証するテストへ更新する
  - `e2e/advanced-quiz-features.spec.ts` の模擬試験・フラッシュカードE2Eの開始手順を、既プレイ状態を経由する手順へ更新する
  - **完了状態**: 3本のE2Eスペックがいずれも新しいゲーティング条件のもとでPlaywrightグリーンとなること
  - _Requirements: 2.3, 2.4, 19.1, 19.2, 19.9, 27.4, 27.5_
  - _Depends: 29.2_
  - _Boundary: Testing_

- [x] 29.5 Phase 37 統合検証
  - 水平思考・早押し形式クイズの既存プレイ・結果保存フローに回帰がないことを確認する
  - 通常モードのプレイ完了 → 結果画面「もう一度プレイする」→ 詳細画面 → 代替導線という一連の導線が途切れず機能することを確認する
  - 模擬試験・フラッシュカードのプレイ進行・結果保存ロジック自体（`quizetika-core` 依存部分）に変更がないことを確認する
  - カスタムクイズ機能（`/my-quiz`）のセッション・UIに影響がないことを確認する
  - **完了状態**: 要件 19.7–19.8・27.6–27.8 を満たし、既存プレイフロー（通常・模擬試験・フラッシュカード・水平思考・早押し・my-quiz）の関連テストスイートに回帰がないこと
  - _Requirements: 2.3, 2.3a, 2.3b, 2.4, 19.7, 19.8, 27.6, 27.7, 27.8_
  - _Depends: 29.3, 29.4_
  - _Boundary: Integration_

## Implementation Notes (Phase 37)

- 専用コンポーネントは作らず `QuizDetailClient` 内インライン実装とする（design.md Phase 37 §3、Simplification原則）。
- 既プレイ判定は新規実装せず、既存 `usePlayedQuizIds` / `hasPlayedThisQuiz` をそのまま再利用する。
- **重要**: 現行のランキング非対象警告は `!hasPlayedThisQuiz`（未プレイ時のみ表示）が条件であり、既存テストもこれに整合している。本フェーズはこの条件を意図的に反転させるため、既存テスト（`tests/components/quiz-detail-client.test.tsx`・E2E3本）の期待値修正が実装の一部として必須（29.3・29.4）。あわせて警告文言のプレイ順序依存の一文も除去する（29.2）。
- **重要**: `modeOption`/`modeSelected`/`modeHeader`/`modeDesc` は水平思考・早押し形式の既存の単一モード表示カードでも共用されているため**削除しない**。3択UIブロック（通常形式の3分岐）のみを削除する（design.md Phase 37 §1・§4 のサニティレビューで確認済み）。
- カスタムクイズ機能（`/my-quiz`）・模擬試験/フラッシュカードのプレイ進行ロジック・`quizetika-core` のLB登録判定は対象外。
- **学び（29.5統合検証）**: `プレイを開始する` → `プレイ` へのボタン文言変更（29.1）は、29.4で更新対象とした3ファイル以外にも波及していた（`e2e/ads.spec.ts`、`e2e/moderation-feedback.spec.ts`）。UI文言変更を伴うタスクでは、変更対象コンポーネントの直接テストだけでなく、リポジトリ全体でのリテラル文字列grep（`e2e/`・`tests/`全体）による回帰確認が必須。

---

### 30. Phase 38: クイズ単位リーダーボードの自分の順位表示・既存データソース是正（2026-07-07）

- [x] 30.1 自分の記録取得・順位算出サービス関数の実装
  - `LeaderboardRecord`を拡張したrank付きの型を追加する
  - クイズID・ボード種別・ユーザーIDを指定して、そのユーザー自身のリーダーボード記録1件と順位を取得する読み取り専用関数を実装する（記録が存在しない場合はnullを返す）
  - 順位は、既存の順位規則（正解数の多い順、同点時は合計解答時間の短い順）に従い、自分より真に上位の記録件数+1として算出する
  - 正解数・合計解答時間が完全に一致する記録同士には同一の順位を付与する
  - 新しい順位規則は考案せず、既存の比較規則をそのまま適用する
  - ローカルSupabase環境に複数件の記録を用意し、順位カウントの絞り込み条件（自分より真に上位、および同順位の判定）が期待どおりの件数・順位を返すことを実際のクエリで確認する（モックテストだけでは条件式自体の正しさを保証できないため）
  - **完了状態**: 指定したクイズ・ボード・ユーザーに対して、記録がある場合は正しいrank付きの記録を、記録がない場合はnullを返す関数が`src/services/attempt.ts`に実装され、ローカルSupabase環境での実クエリ確認で期待どおりの順位・同順位が得られること
  - _Requirements: 9.10, 9.11, 9.13, 9.15_
  - _Boundary: AttemptService_

- [x] 30.2 (P) 自分の順位取得サービス関数の単体テスト
  - 自分の記録が存在する場合に正しい順位が返ることを検証するテストを追加する
  - 自分の記録が存在しない場合にnullが返ることを検証するテストを追加する
  - 自分と正解数・合計解答時間が完全に一致する他ユーザーが存在する場合、同一順位になることを検証するテストを追加する
  - 自分より真に上位の記録のみが順位カウントに含まれ、自分自身の記録がカウントに含まれないことを検証するテストを追加する
  - **完了状態**: `tests/services/attempt-leaderboard.test.ts`に上記ケースのテストが追加され、すべてグリーンであること
  - _Requirements: 9.11, 9.13_
  - _Depends: 30.1_
  - _Boundary: Testing_

- [x] 30.3 (P) クイズリーダーボード統合取得フックの実装
  - クイズIDとユーザーID（未ログイン時はnull）を受け取り、初回プレイ・リプレイ双方のボードについて、上位5件と自分の順位を取得するフックを実装する
  - 両ボードの上位5件取得は常に並行して行う
  - ユーザーIDが非nullの場合のみ、両ボードの自分の順位取得を並行して行う。未ログイン時はこれらの問い合わせを発行しない
  - クイズIDまたはユーザーIDが変化した場合は再取得する
  - **完了状態**: フックがログイン済みユーザーに対して初回・リプレイ両ボードの上位5件と自分の順位（rankまたはnull）を返し、未ログイン時は自分の順位の問い合わせ自体を発行しないことが確認できること
  - _Requirements: 9.9, 9.10, 9.12_
  - _Depends: 30.1_
  - _Boundary: useQuizLeaderboard_

- [x] 30.4 (P) クイズリーダーボード統合取得フックの単体テスト
  - 認証済みユーザーで両ボードの上位5件・自分の順位が取得されることを検証するテストを追加する
  - 未ログイン時に自分の順位の問い合わせが発行されないことを検証するテストを追加する
  - 一方のボードに自分の記録がなくもう一方のボードの結果に影響しないことを検証するテストを追加する
  - **完了状態**: `tests/hooks/useQuizLeaderboard.test.ts`が新規作成され、上記ケースがすべてグリーンであること
  - _Requirements: 9.9, 9.10, 9.11, 9.12_
  - _Depends: 30.3_
  - _Boundary: Testing_

- [x] 30.5 (P) クイズ詳細リーダーボード表示の自分の順位対応・データソース刷新
  - クイズ詳細画面のリーダーボード表示コンポーネントを、破損した既存データソースへの依存から、統合取得フックを利用する実装へ切り替える
  - 初回プレイ・リプレイそれぞれのタブで、上位5件の表とは別枠に、ログインユーザー自身の順位行を常に表示する（上位5件に含まれる場合も省略しない）
  - 自分の順位行は、上位5件の表と同じ列構成（順位・ユーザー表示名・正解数・合計解答時間・達成日）で表示する
  - 自分の記録がないボードでは、自分の順位欄を表示しない
  - 未ログイン（ゲスト）の場合は、いずれのボードでも自分の順位欄を表示せず、上位5件の表のみを表示する
  - 自分の順位行に、初回プレイ側・リプレイ側を区別できるE2E用の識別子を付与する
  - 既存の空状態表示、タブ切替、上位5件表のE2E用識別子は維持する
  - **完了状態**: 実際に保存されたリーダーボード記録がある状態でクイズ詳細画面を表示すると、上位5件が正しく表示され（従来は常に空表示だった不具合が解消され）、ログイン済みユーザーが記録を持つボードでは自分の順位行が別枠で表示されること
  - _Requirements: 9.1, 9.2, 9.3, 9.4, 9.5, 9.6, 9.7, 9.8, 9.9, 9.10, 9.11, 9.12, 9.14_
  - _Depends: 30.3_
  - _Boundary: QuizDualLeaderboard_

- [x] 30.6 (P) クイズ詳細リーダーボードのコンポーネントテスト刷新
  - 既存の「クイズオブジェクトへの直接データ注入」によるテストを、統合取得フックの結果をモックする形式へ全面的に書き換える
  - 上位5件の表示、自分の順位行（圏内・圏外双方）、記録なし時の非表示、ゲスト時の非表示をそれぞれ検証するテストを追加する
  - **完了状態**: `tests/components/quiz-dual-leaderboard.test.tsx`が新しいデータ取得方式に整合した形で全面刷新され、すべてグリーンであること
  - _Requirements: 9.1, 9.4, 9.9, 9.10, 9.11, 9.12, 9.14_
  - _Depends: 30.5_
  - _Boundary: Testing_

- [x] 30.7 (P) クイズ詳細リーダーボードのE2Eテスト更新
  - 実際にクイズをプレイして記録を作成したうえで、クイズ詳細画面の上位5件表示が正しく機能することを検証するテストを追加または更新する
  - ログイン済みユーザーが自分の順位行を確認できることを検証するテストを追加する
  - **完了状態**: リーダーボード関連のE2Eスペックが実データに基づいて上位5件表示・自分の順位表示の双方を検証し、Playwrightでグリーンであること
  - _Requirements: 9.5, 9.6, 9.9, 9.14_
  - _Depends: 30.5_
  - _Boundary: Testing_

- [x] 30.8 Phase 38 統合検証
  - 記録がないクイズ・記録が上位5件圏内のクイズ・圏外のクイズ・未ログイン状態のそれぞれでクイズ詳細画面を確認し、要件どおりの表示になることを確認する
  - 初回プレイ・リプレイ両方のタブで自分の順位が独立して評価されることを確認する
  - 既存の上位5件表示・タブ切替・空状態の挙動に回帰がないことを確認する
  - **完了状態**: 要件9.1〜9.15のすべてを満たし、関連するユニット・コンポーネント・E2Eテストスイートに回帰がないこと
  - _Requirements: 9.1, 9.2, 9.3, 9.4, 9.5, 9.6, 9.7, 9.8, 9.9, 9.10, 9.11, 9.12, 9.13, 9.14, 9.15_
  - _Depends: 30.2, 30.4, 30.6, 30.7_
  - _Boundary: Integration_

## Implementation Notes (Phase 38)

- 既存の`Quiz.leaderboardFirstPlay`/`leaderboardReplay`/`leaderboard`フィールドおよび関連フィクスチャ（`quiz-editor.tsx`, `quiz-play-client.tsx`, `test-play.ts`等）の型変更・削除は本フェーズのスコープ外とする。無害な死んだ参照経路として残置し、将来のクリーンアップに委ねる（design.md Phase 38 Non-Goals参照）。
- 自分の順位算出クエリはクライアント合成クエリ（2クエリ構成）で実装し、新規RPCは追加しない。RPC化は将来の最適化候補とする。
- 自分の順位算出における同順位カウントクエリの条件式は、常に数値型の値のみを埋め込み、外部入力文字列を直接連結しない。
- **重要**: 30.1では、モックテスト（30.2）に加えて、ローカルSupabase環境での実クエリ検証を必須とする。`.or()`によるフィルタ条件式はこのコードベース初出のパターンであり、構文誤りがあっても常に0件ヒット（誤った順位）という形で静かに失敗しうるため、モックだけでは条件式自体の正しさを保証できない。
- クイズ詳細画面のリーダーボード表示コンポーネントの`quiz` propの廃止は、`src/app/quiz/[id]/page.tsx`以外に呼び出し元がないことを前提とする。30.5着手時に他の呼び出し元がないことを再確認する。
- **学び（30.5実装）**: `quiz` prop廃止に伴い、既存`tests/components/quiz-dual-leaderboard.test.tsx`（旧`quiz` prop直接注入方式）はビルドを壊すため、`describe.skip`で一時的に無効化し30.6（既存テストの全面書き換え）へ明示的に引き継いだ（コメントでタスク番号を明記、レビューで正当な段階的引き継ぎと確認済み）。30.6はこのファイルの書き換えを必ず行うこと（skipしたまま放置しない）。あわせて、要件9.5の「`leaderboardFirstPlay`が空のときの`leaderboard`レガシーフォールバック」は、データソースが`leaderboard_entries`（`getLeaderboard`）に一本化されたことで意味を失っており、30.6/30.8で「サービス層で意味を保つのか、正式に廃止と明記するのか」を確認すること。
- **学び（30.8統合検証）**: 上記の要件9.5レガシーフォールバック懸念を`src/services/quiz.ts`の`mapRowToQuiz`まで遡って調査した結果、`Quiz.leaderboard`（レガシーフィールド）はSupabase移行後一度もDB列にマッピングされておらず常に`undefined`だったこと、かつ移行前の`leaderboardFirstPlay`自体も存在しない列参照で常に`[]`だったことを確認した（`research.md`のPhase 38ギャップ分析と整合）。すなわちこのフォールバック経路は現行バックエンドで一度も実データを返したことがなく、`requirements.md`9.5/9.6本文の「レガシーフォールバック」記述はFirestore時代に意図された設計が実現しないまま残った**文書の記述漏れ**であり、機能的欠陥ではないと判定した。`requirements.md`の9.5/9.6文言は実態（`leaderboard_entries`一本化）に合わせて将来更新することを推奨するが、非ブロッキング。
- **学び（30.8統合検証）**: `npm run test`をPowerShellで`2>&1`パイプ経由実行すると、Jest自体が全緑でも`NativeCommandError`によりexit code 1が返る既知の挙動がある。実際のPASS/FAIL判定は末尾の`Test Suites:`/`Tests:`サマリー行を見ること（コマンドのexit codeだけで判断しない）。
- **学び（30.8後: 複数アカウントE2E追加）**: `e2e/leaderboard.spec.ts`に、合成競合アカウント6件（`leaderboard_entries`へ直接シード）でTOP5を埋め、自分の記録を確定的に圏外へ追いやる複合テストを追加した。**注意点1**: 自動化ブラウザ操作は実測`elapsed_seconds`がほぼ0秒になり、素朴に「速いシード値」で圏外を狙うと自分の記録が逆に最速（#1位）になってしまう。自分の記録をDB直接`UPDATE`で確定的に遅い値へ調整すること（本物の`saveAttempt`経路で記録を作成した後に調整するため、書き込み経路自体の検証は損なわれない）。**注意点2**: `seedOtherAuthorQuiz`のクイズ説明文に「リーダーボード」という語を含めると、F-801（`text=リーダーボード`という緩いロケータを使う既存テスト、Phase 38とは無関係）がホーム画面に蓄積したテスト用クイズカードを誤クリックしてしまう。フィクスチャの説明文に「リーダーボード」という語を含めないこと。**注意点3**: `[TEST]`接頭辞のクイズは`global-setup.ts`の自動クリーンアップ対象外（`[AD_TEST]`のみ削除）のため、ローカルでの反復実行では手動でDBから削除する必要がある。

---

### 31. Phase 39: クイズ詳細画面のSNS共有（2026-07-16）

- [x] 31.1 共有URL生成関数の実装
  - クイズタイトルと共有URLからXの投稿作成インテントURLを組み立てる純粋関数を実装する（タイトル・URLは`encodeURIComponent`でエンコードする）
  - 共有URLからLINEの共有インテントURLを組み立てる純粋関数を実装する（URLは`encodeURIComponent`でエンコードする）
  - 投稿完了画面の既存共有実装と同一のURL組み立て規則に従う（新規パターンを考案しない）
  - **完了状態**: 任意のタイトル・URLを渡すと、正しくエンコードされたX投稿作成URLおよびLINE共有URLを返す純粋関数が実装されていること
  - _Requirements: 28.2, 28.3, 28.6, 28.7, 28.8_
  - _Boundary: social-share_

- [x] 31.2 (P) 共有URL生成関数の単体テスト
  - タイトル・URLに記号や日本語が含まれる場合でも正しくエンコードされることを検証するテストを追加する
  - 生成されたURLがXの投稿作成インテント形式であることを検証するテストを追加する
  - 生成されたURLがLINEの共有インテント形式であることを検証するテストを追加する
  - **完了状態**: 共有URL生成関数のテストファイルが追加され、すべてグリーンであること
  - _Requirements: 28.2, 28.3, 28.7_
  - _Depends: 31.1_
  - _Boundary: Testing_

- [x] 31.3 (P) クイズ詳細共有セクションUIコンポーネントの実装
  - クイズIDとタイトルを受け取り、X共有・LINE共有・URLコピーの3つの共有操作を表示するコンポーネントを実装する
  - 共有対象URLは、クライアント側でクイズ詳細画面自身のURL（オリジン＋クイズID）として算出する
  - URLコピー操作でクリップボードへのコピーを行い、成功時のみコピー完了フィードバックを表示し、3秒後に自動的に非表示へ戻す
  - クリップボードへのコピーに失敗した場合は、コピー完了フィードバックを表示しない
  - コンポーネントのアンマウント時にフィードバック表示用のタイマーを解除する
  - 共有セクション全体にE2E識別用の属性を付与する
  - X・LINE・URLコピー以外の共有チャネルや、共有アクションの分析・トラッキング処理は追加しない
  - **完了状態**: クイズID・タイトルを渡すと、X共有・LINE共有・URLコピーの3操作を描画し、URLコピー操作が視覚的な成功フィードバックを伴うコンポーネントが実装されていること
  - _Requirements: 28.1, 28.4, 28.5, 28.6, 28.7, 28.8, 28.9, 28.10, 28.11_
  - _Depends: 31.1_
  - _Boundary: QuizShareSection_

- [x] 31.4 (P) 共有セクションのコンポーネントテスト
  - X共有・LINE共有のリンクが新しいタブで開く属性（外部リンクの安全属性を含む）を持つことを検証するテストを追加する
  - URLコピー成功時にコピー完了フィードバックが表示され、一定時間後に非表示になることを検証するテストを追加する（タイマーはモックを使用する）
  - クリップボードへのコピーが失敗した場合、コピー完了フィードバックが表示されないことを検証するテストを追加する
  - **完了状態**: 共有セクションコンポーネントのテストファイルが追加され、すべてグリーンであること
  - _Requirements: 28.4, 28.5_
  - _Depends: 31.3_
  - _Boundary: Testing_

- [x] 31.5 (P) クイズ詳細画面への共有セクション統合
  - クイズ詳細画面のコンテンツ内に共有セクションコンポーネントを配置し、表示中のクイズのIDとタイトルを渡す
  - **完了状態**: クイズ詳細画面（`/quiz/[id]`）を表示すると、当該クイズの共有セクションが描画されること
  - _Requirements: 28.1_
  - _Depends: 31.3_
  - _Boundary: QuizDetailClient_

- [x] 31.6 (P) クイズ詳細画面コンポーネントテストの更新
  - クイズ詳細画面のレンダリングテストに、共有セクションが描画されることを検証するケースを追加する
  - **完了状態**: クイズ詳細画面のコンポーネントテストが更新され、共有セクションの描画を含めてすべてグリーンであること
  - _Requirements: 28.1_
  - _Depends: 31.5_
  - _Boundary: Testing_

- [x] 31.7 (P) 既存E2Eソフトアサーションの是正
  - クイズ詳細画面のSNS共有機能を検証する既存E2Eテストが、要素の有無で検証をスキップする条件付きアサーションになっている状態を是正する
  - 共有セクションのE2E識別用属性を用いて、共有セクションが確実に表示されることを検証する
  - X共有リンクのリンク先URLがX（Twitter）の投稿作成インテントであること、LINE共有リンクのリンク先URLがLINEの共有インテントであることをそれぞれ直接検証する
  - **完了状態**: 対象のE2Eテストが、機能が存在しない状態では失敗し、実装後は確実にグリーンになる検証内容へ更新されていること
  - _Requirements: 28.1, 28.2, 28.3, 28.11_
  - _Depends: 31.5_
  - _Boundary: Testing_

- [x] 31.8 Phase 39 統合検証
  - クイズ詳細画面を表示し、X共有・LINE共有・URLコピーの3操作がすべて機能することを確認する
  - 投稿完了画面の既存共有実装と挙動（共有先・コピー成功時フィードバック）が一致していることを確認する
  - 投稿完了画面自体のファイルに変更が加えられていないことを確認する
  - **完了状態**: 要件28.1〜28.11のすべてを満たし、関連するユニット・コンポーネント・E2Eテストスイートに回帰がないこと
  - _Requirements: 28.1, 28.2, 28.3, 28.4, 28.5, 28.6, 28.7, 28.8, 28.9, 28.10, 28.11_
  - _Depends: 31.2, 31.4, 31.6, 31.7_
  - _Boundary: Integration_

## Implementation Notes (Phase 39)

- 共有URL生成ロジック（`src/lib/social-share.ts`）は投稿完了画面（`SuccessClient`）の既存実装と同一のURL組み立て規則を踏襲し、`SuccessClient`自体は一切変更していない（`git log`で確認済み、要件28.8境界を厳守）。
- 31.8統合検証で`e2e/seo-sharing.spec.ts`内の既存テスト「クイズ結果画面: SNS共有機能が正常に動作すること」が失敗することを発見した。調査の結果、これはプレイ→結果画面フローの選択肢ボタンロケータに起因する**Phase 39と無関係な既存の不具合**であり、本フェーズのどのコミットもこのテストコードや関連する詳細画面のプレイ導線マークアップを変更していないことを`git show`で確認済み（非ブロッキング、別途対応が必要）。

## Implementation Notes (Phase 39)

- 共有URL生成ロジック（`src/lib/social-share.ts`、design.md Phase 39参照）は、投稿完了画面（`SuccessClient`、`quizetika-ui-quiz-lifecycle`所有）の既存実装と同一のURL組み立て規則に従うが、`SuccessClient`自体のファイルは本フェーズで変更しない（境界）。
- 31.1完了後、31.2（単体テスト）と31.3（UIコンポーネント実装）は異なる境界（`social-share`と`QuizShareSection`）のため並行着手可能。同様に31.3完了後の31.4・31.5、31.5完了後の31.6・31.7もそれぞれ異なる境界のため並行着手可能。
- 31.7で是正する既存E2Eテスト（`e2e/seo-sharing.spec.ts`のF-703）は、共有機能が存在しない現状でも要素の可視性チェックが`false`を返すことで無検証のままパスしてしまう状態にある（`research.md` Phase 39 Research Log参照）。是正時はこの条件付きスキップを除去し、要素が存在しない場合はテストが失敗するようにすること。

---

### 32. Phase 40: クイズ詳細共有UIのアイコン+メニュー方式への改定（2026-07-16）

- [x] 32.1 QuizShareSectionのドロップダウンメニュー化
  - 常時表示のX共有・LINE共有・URLコピーの3ボタンを廃止し、共有アイコンボタン+クリックで展開するドロップダウンメニュー（3項目）の構成へ置き換える
  - アイコンボタン押下でメニューを開き、再度押下・メニュー外クリック・Escapeキーでメニューを閉じる
  - X共有・LINE共有のメニュー項目は選択時に新しいタブでリンク先を開き、メニューを閉じる
  - URLコピーのメニュー項目は選択してもメニューを閉じず、ラベルをコピー完了を示す文言に一定時間差し替えたのち、自動的にメニューを閉じる
  - クリップボードへのコピーに失敗した場合はラベルを差し替えない
  - 共有対象URL・共有先の遷移ロジック（`social-share.ts`の利用）はPhase 39の実装を変更しない
  - トリガーおよびメニューにE2E識別用の属性を付与する
  - **完了状態**: 共有アイコンボタンを押すとX共有・LINE共有・URLコピーの3項目を含むメニューが表示され、URLコピー選択時はメニューが開いたままフィードバックを示し一定時間後に自動的に閉じるコンポーネントが実装されていること
  - _Requirements: 28.1, 28.1a, 28.1b, 28.1c, 28.4, 28.5, 28.11_
  - _Boundary: QuizShareSection_

- [x] 32.2 (P) QuizShareSectionコンポーネントテストの更新
  - トリガー押下でメニューが表示され、初期状態では非表示であることを検証するテストを追加する
  - メニュー内のX/LINE項目のリンク属性（href・新しいタブで開く属性）を検証するテストを追加する
  - URLコピー項目選択時にメニューが開いたままラベルが差し替わり、一定時間後に自動的にメニューが閉じることを検証するテストを追加する（タイマーはモックを使用する）
  - クリップボードへのコピーが失敗した場合、ラベルが差し替わらないことを検証するテストを追加する
  - **完了状態**: 更新後の共有セクションコンポーネントテストがすべてグリーンであること
  - _Requirements: 28.1, 28.1a, 28.1b, 28.1c, 28.4, 28.5_
  - _Depends: 32.1_
  - _Boundary: Testing_

- [x] 32.3 (P) クイズ詳細画面ヘッダーへの共有アイコン配置
  - クイズ詳細画面のブックマークボタンの隣に共有アイコンボタンを配置する
  - 説明セクション直下に配置されていた旧来の常時表示共有セクションの配置を除去する
  - **完了状態**: クイズ詳細画面（`/quiz/[id]`）のヘッダー内で、ブックマークボタンの隣に共有アイコンボタンが表示されること
  - _Requirements: 28.1_
  - _Depends: 32.1_
  - _Boundary: QuizDetailClient_

- [x] 32.4 (P) クイズ詳細画面コンポーネントテストの更新
  - クイズ詳細画面のレンダリングテストにおける共有セクションの検証を、新しいトリガー要素の描画検証へ更新する
  - **完了状態**: 更新後のクイズ詳細画面コンポーネントテストがすべてグリーンであること
  - _Requirements: 28.1_
  - _Depends: 32.3_
  - _Boundary: Testing_

- [x] 32.5 (P) 既存E2Eテストのメニュー方式への追従
  - クイズ詳細画面のSNS共有機能を検証する既存E2Eテストを、共有アイコンボタンのクリックによるメニュー展開を経た検証手順へ更新する
  - X共有・LINE共有リンクのリンク先URLの直接検証は維持する
  - **完了状態**: 対象のE2Eテストが、メニュー展開を経て共有リンクを検証する内容に更新され、グリーンであること
  - _Requirements: 28.1, 28.1a, 28.2, 28.3, 28.11_
  - _Depends: 32.3_
  - _Boundary: Testing_

- [x] 32.6 Phase 40 統合検証
  - クイズ詳細画面を表示し、共有アイコンボタンの押下でメニューが展開し、X共有・LINE共有・URLコピーの3操作がすべて機能することを確認する
  - メニュー外クリック・Escapeキー・アイコン再押下でメニューが閉じることを確認する
  - 旧来の常時表示共有セクションが画面上に残っていないことを確認する
  - **完了状態**: 要件28.1〜28.11のすべてを満たし、関連するユニット・コンポーネント・E2Eテストスイートに回帰がないこと
  - _Requirements: 28.1, 28.1a, 28.1b, 28.1c, 28.2, 28.3, 28.4, 28.5, 28.6, 28.7, 28.8, 28.9, 28.10, 28.11_
  - _Depends: 32.2, 32.4, 32.5_
  - _Boundary: Integration_

## Implementation Notes (Phase 40)

- Base UI `Menu.Item` には `closeOnClick?: boolean`（既定 `true`）という正規のプロパティが存在する。design.mdのPhase 40初版はこのプロパティの存在を把握しておらず`event.preventDefault()`ワークアラウンドを想定していたが、32.1実装時に発見し、より単純な`closeOnClick={false}`方式へ差し替えた（design.md該当箇所は修正済み）。
- 32.6統合検証で、Phase 39時代の残存`data-testid="quiz-detail-share-section"`（コンテナのラッパーdiv、機能的には無害だが仕様上「除去済みであること」を明示的に要求されていた）を発見し、コンポーネントおよび対応するテストアサーションから除去した。
- 32.6検証時、`e2e/seo-sharing.spec.ts`の「クイズ結果画面: SNS共有機能が正常に動作すること」テストが**別の原因**で再度不安定であることを発見した（31.8で発見・修正した`optionBtn`セレクタ問題とは異なり、今回は「プレイ」ボタン押下後に`/play`へ遷移しない、または選択肢のradio要素が出現しないという症状で、実行のたびに失敗箇所が変わる）。Phase 39・Phase 40のいずれとも無関係な、テストのクイズ選定方法に起因する既存の不安定性と判明したため、ユーザーの指示により追加で修正した。
- **不安定性の修正（Phase 40完了後の追加対応）**: 根本原因は2つ複合していた。(1) `ensureQuizAndNavigate`ヘルパーがホーム画面の「最初のクイズカード」を無条件に採用する設計になっており、他のE2Eテストが作成する多様な出題形式（早押し・ウミガメのスープ等）がたまたま「最初」に来ると、単一選択式クイズを前提とするこのテストの後続手順が成立しなくなる → 対応: このテスト専用に、常に単一選択式クイズを新規作成してから遷移する`createSingleChoiceQuizAndNavigate`ヘルパーを追加。(2) 「プレイ」ボタンのロケータ`page.locator('button').filter({ hasText: /プレイ|始める/ })`が曖昧で、クイズランキング欄の「初回プレイ」タブボタン（`role="tab"`、こちらも「プレイ」を含む）とも一致してしまい、Suspenseによる非同期レンダリング順序次第でどちらが先にDOMへ現れるかが変動していた → 対応: `data-analytics="quiz-play-start-detail"`属性による確定的なロケータへ変更。修正後、対象テストを独立に4回連続実行してすべてグリーンであることを確認し、フルスイート（10件）にも回帰がないことを確認済み。