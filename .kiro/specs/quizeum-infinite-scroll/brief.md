# Brief: quizetika-infinite-scroll
...
- **Firestoreカーソルクエリ `getQuizzesByAuthorPage` の実装**: `orderBy('createdAt', 'desc')`、`startAfter` と `limit(20)` をサポートするクイズ取得関数を `quizetika-core` に追加し、カーソル定義に `'author'` を加える。
...
## Upstream / Downstream
- **Upstream**: `quizetika-core` (`getQuizzesByAuthorPage`, `QuizFeedTabKind` 拡張), `quizetika-ads` (広告状態/コンポーネント)
- **Downstream**: UI各画面 (`quizetika-play-flow-ui`, `quizetika-auth-profile-ui`)

## Existing Spec Touchpoints
- **Extends**: なし (新規スペックとしての定義)
- **Adjacent**: `quizetika-play-flow-ui` (検索画面), `quizetika-auth-profile-ui` (プロフィール画面)
