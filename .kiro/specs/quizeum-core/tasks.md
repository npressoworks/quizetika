...
### 28. Phase 39: NGワードマスタ参照によるコンテンツ検証への移行 (2026-07)

- [x] 28.1 (P) NGワード読み取りサービスの新規実装
  - `ng-words.ts` に、`supabase-governance` が所有する `ng_words` マスタから有効な語句一覧のみを取得する読み取り専用関数を実装する（マスタ自体の登録・編集・無効化はここでは行わない）
  - 取得に失敗した場合は例外をそのまま呼び出し元へ伝播させる
  - **完了状態**: 関数呼び出しが `ng_words` の有効な語句配列を返し、Supabaseクエリが失敗した場合は例外がスローされること
  - _Requirements: 32.1, 32.4, 32.5_
  - _Boundary: NgWordsService_

- [x] 28.2 (P) クイズ公開バリデーションのNGワード判定を引数化
  - `quiz-validation.ts` のハードコードされたNGワード配列定数を削除する
  - 禁止語判定関数とクイズ公開バリデーション関数を、NGワード一覧を引数として受け取る形へシグネチャ変更する（純粋関数としての性質は維持する）
  - **完了状態**: 渡されたNGワード一覧に含まれる語句のみが検出され、一覧にない語句は検出されないこと
  - _Requirements: 32.1, 32.2_
  - _Boundary: QuizValidation_

- [x] 28.3 (P) クイズ公開サービスへのNGワードマスタ参照統合
  - クイズ公開処理内でNGワード読み取りサービスを呼び出し、取得した一覧を公開バリデーションへ渡すよう統合する
  - NGワード一覧の取得に失敗した場合は公開処理をエラーとして中断し、未検証のまま公開しない
  - **完了状態**: NGワードマスタに登録済みの語句を含むクイズの公開がバリデーションエラーで拒否され、マスタ取得失敗時にもクイズが公開されないこと
  - _Requirements: 32.1, 32.2, 32.3, 32.4_
  - _Depends: 28.1, 28.2_
  - _Boundary: QuizService_

- [x] 28.4 (P) クイズエディタのクライアント側事前検証への統合
  - クイズエディタの公開前クライアントサイド検証で、NGワード一覧を取得し禁止語チェックに用いるよう更新する
  - 一覧取得に失敗した場合は事前チェックをスキップし、最終防衛線であるサーバー側検証に委ねる
  - **完了状態**: NGワードを含む問題文を入力した状態で公開操作を行うと、クライアント側で送信前に警告が表示されること
  - _Requirements: 32.1, 32.2_
  - _Depends: 28.1, 28.2_
  - _Boundary: QuizEditor_

- [x] 28.5 Phase 39 単体・結合テストの追加・更新
  - 禁止語判定関数・公開バリデーション関数の単体テストを、引数化されたNGワード一覧に基づく判定へ書き換える
  - クイズ公開サービスの結合テストで、NGワード一覧取得失敗時に公開が拒否されること、マスタに新規登録された語句が次回の公開検証から反映されることを検証する
  - **完了状態**: Phase 39 に関連するすべての Jest テストがグリーンでパスすること
  - _Requirements: 32.1, 32.2, 32.3, 32.4_
  - _Depends: 28.3, 28.4_
  - _Boundary: Testing_

- [x] 28.6 Phase 39 統合検証
  - コア全体のテストスイートを実行し、ビルドや既存機能への回帰がないことを確認する
  - **完了状態**: 全ての Jest テストがグリーンでパスし、`npm run build` がエラーなく成功すること
...
## Phase 44: リッチダッシュボード向け集計データ提供と試行ライフサイクル記録

- [ ] 31. Foundation: マイグレーションと RPC 群
- [ ] 31.1 試行ライフサイクル記録の DB 基盤
  - 試行テーブルへ開始時刻・進行位置の列を追加し、クイズ×完了日時の複合インデックスを作成する
  - 複数問一括モード（normal/exam/flashcard/review）用の開始記録 RPC と進行反映 RPC を新設する（1問単位契約の my-quiz・非永続の test-play は対象外）
  - 既存の試行保存 RPC に後方互換の試行 ID 引数（DEFAULT NULL）を追加し、指定時は該当未完了行を完了状態へ更新、未指定・該当なしは従来どおり新規挿入とする（既存の二重検証・リーダーボード振り分け・プレイ数加算は両パスで維持）
  - ウミガメの開始 RPC にも開始時刻の設定を追加する
  - 完了条件: ローカル Supabase の db reset が成功し、開始→進行→完了で同一行が完了状態になること、試行 ID 未指定の従来呼び出しが無変更で通ることを SQL レベルで確認できる
  - _Requirements: 39.1, 39.2, 39.3, 39.5, 39.7, 39.10_
  - _Boundary: attempts スキーマ・ライフサイクル RPC_

- [ ] 31.2 プレイヤー集計・ドリルダウン RPC
  - 認証ユーザー本人の全期間試行を母集団に、期間（7日/30日/90日/全期間）・ジャンル・タグ・設問形式・モードの AND フィルタ付き集計を JSONB で一括返却する RPC を新設する（KPI・JST 基準の粒度別推移・内訳・得意苦手・ワードクラウド用タグ/タイトル集計・ストリーク）
  - 形式フィルタ時の母集団定義（試行単位指標＝該当形式を1問以上含む完了試行、解答単位指標＝該当形式の解答明細のみ）を design どおり実装する
  - 同一フィルタ条件のプレイ履歴一覧をカーソルページングで返す RPC を新設する
  - 未完了試行・非永続モードを集計から除外する
  - 完了条件: SECURITY INVOKER + RLS により他ユーザーの集計・履歴が取得できず、フィルタ組み合わせごとの集計値が手計算のシードデータと一致する
  - _Requirements: 37.1, 37.2, 37.3, 37.4, 37.5, 37.6, 37.7, 37.8, 37.9, 37.10, 37.11, 37.12, 37.13, 38.1, 38.4_
  - _Boundary: プレイヤー集計 RPC_
  - _Depends: 31.1_

- [ ] 31.3 クリエイター集計・クイズ単体分析 RPC
  - 作成者本人検証付きの SECURITY DEFINER RPC を新設し、自作クイズ群の期間・ジャンル・出題形式・公開状態フィルタ付き集計（期間 KPI・ユニークプレイヤー数・完走率・トレンド・クイズ別ランキング・設問形式別）を集計値のみで返す
  - クイズ単体のスコア分布（期間連動）・離脱分布・完走率を返す RPC を新設する
  - 完走率・離脱分布は開始時刻が記録された世代の試行のみを母集団とし、サンプル件数も返す
  - DEFINER 共通ハードニング（search_path 固定・未認証拒否・anon の EXECUTE 剥奪・個別プレイヤー情報の非露出）を適用する
  - 完了条件: 他ユーザー ID での呼び出しが例外となり、戻り値にユーザー識別子・表示名が含まれず、世代識別（開始時刻 NULL 行の除外）が SQL レベルで確認できる
  - _Requirements: 40.1, 40.2, 40.3, 40.4, 40.5, 40.6, 40.7, 40.8, 40.9, 40.10, 40.11, 41.2, 41.3, 41.4, 41.5, 41.6_
  - _Boundary: クリエイター集計 RPC_
  - _Depends: 31.1_

- [ ] 31.4 型再生成とマイグレーション統合検証
  - ローカル Supabase 起動状態で型定義を再生成し、新 RPC 群がクライアント型に反映されることを確認する
  - db reset → シードデータ投入 → 各 RPC の疎通・認可・世代識別を一括確認する
  - 完了条件: `npm run gen:types` 後に `npm run build` が成功し、SQL レベルの検証結果を Implementation Notes に記録できる
  - _Requirements: 39.1, 40.10_
  - _Depends: 31.1, 31.2, 31.3_

- [ ] 32. Core: サービス層契約
- [ ] 32.1 (P) ダッシュボード型契約とプレイヤー集計サービス
  - フィルタ・集計結果の型契約（期間区分・フィルタ・KPI・推移・内訳・ドリルダウン等）を新設し、`any` を使わず定義する
  - プレイヤー統計取得・ドリルダウン履歴取得・試行明細取得のサービス関数を実装する（snake→camel 変換、既存サービスと同一のエラー封筒）
  - 解答明細のない旧試行の明細取得は要約のみ + 明細なしを判別可能な形で返す
  - 完了条件: rpc モックによるユニットテストで引数マッピング・型変換・エラー文言・明細なしフォールバックが検証される
  - _Requirements: 37.1, 37.4, 38.1, 38.2, 38.3, 38.4_
  - _Boundary: dashboard サービス（プレイヤー系）_
  - _Depends: 31.4_

- [ ] 32.2 クリエイター集計・クイズ単体分析サービス（32.1 と同一ファイル群を編集するため並行不可）
  - クリエイター統計取得・クイズ単体分析取得のサービス関数を実装する（完走率サンプル件数を含む契約）
  - 設問別の累計正答率・選択肢分布は既存クイズ読み取り（キャッシュカウンタ）で充足されることをテストで固定し、追加実装を行わない
  - 完了条件: rpc モックによるユニットテストでフィルタ引数マッピング・集計結果変換・エラー処理が検証される
  - _Requirements: 40.1, 40.4, 40.6, 41.1, 41.2, 41.3, 41.4_
  - _Boundary: dashboard サービス（クリエイター系）_
  - _Depends: 31.4, 32.1_

- [ ] 32.3 (P) 試行ライフサイクルサービス関数
  - プレイ開始記録（失敗時は null を返しプレイ継続）・進行反映（fire-and-forget で失敗を握りつぶす）・試行 ID 付き保存統合をサービス層に実装する
  - 対象モードの限定（複数問一括モードのみ）とオフライン同期経路（試行 ID なし＝開始・完了同時記録）の維持を実装する
  - セッション再開時に同一試行 ID で完了更新され二重カウントされないことをテストで固定する
  - 完了条件: ユニットテストで開始失敗時の非例外・進行失敗の握りつぶし・保存の UPDATE/INSERT パス分岐・既存二重検証の維持が検証される
  - _Requirements: 39.1, 39.2, 39.3, 39.4, 39.5, 39.6, 39.8, 39.9, 39.10_
  - _Boundary: attempt サービス_
  - _Depends: 31.4_

- [ ] 32.4 プレイヤー統計ライブラリの整理
  - クライアント側集計本体を廃止し、ワードクラウド語抽出（タイトル集計→キーワード展開）等の UI 補助関数のみ残置する（本ファイルの改変は core が単独所有）
  - 完了条件: 補助関数のユニットテストが通り、集計本体への参照が本体コードから消えても `npm run build` が成功する（UI スペック側の追随はビルドを壊さない範囲で最小限の import 修正に留める）
  - _Requirements: 37.9_
  - _Depends: 32.1_

- [ ] 33. Validation: 統合検証
- [ ] 33.1 Phase 44 コア統合検証
  - 全ユニットテストとビルドを実行し、既存の試行保存・リーダーボード振り分け・オフライン同期・プレイ履歴取得（未完了除外）に回帰がないことを確認する
  - 完了条件: `npm run build`・`npm run test` が成功し、既存 attempt 系テストスイートがすべてグリーンである
  - _Requirements: 39.8, 37.11_
  - _Depends: 32.1, 32.2, 32.3, 32.4_

## Implementation Notes (Phase 44)

- 実装順: 31.1 → 31.2 / 31.3（並行可、同一マイグレーションファイルに追記する場合は直列）→ 31.4 → 32.1 / 32.3（並行可）→ 32.2（32.1 と同一ファイルのため直列）→ 32.4 → 33.1。
- マイグレーションは単一ファイル `20260726000000_dashboard_aggregation.sql` に集約する（design.md Phase 44 Data Model 参照）。31.2 / 31.3 を並行実施する場合はファイル分割（連番別ファイル）を許容する。
- 要件 41.1 は既存キャッシュカウンタの再利用で充足（32.2 でテスト固定、追加実装なし）。プレイ画面からのライフサイクル関数呼び出しの組み込みはプレイ画面側スペックの担当（未組み込みでもフォールバック INSERT で保存は成立する縮退設計）。
- UI（quizeum-creator-dash-ui Phase 44）は本フェーズの 32.x 完了後に着手すること。
