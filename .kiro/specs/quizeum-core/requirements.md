...
#### 受け入れ基準
1. 作成者がクイズを下書き保存するとき、Quizeum Systemはタイトル・有効なジャンルマスタから選択されたジャンル・登録済み各設問の問題文（5文字以上500文字以内）が満たされている場合のみ保存を許可し、外部ユーザーが閲覧できないようにすること。
2. 作成者がクイズを下書きまたは公開保存するとき、Quizeum Systemは選択されたジャンルが有効なジャンルマスタに存在することを検証し、存在しない場合は保存を拒否すること。
3. 作成者がクイズを公開したとき、Quizeum Systemは公開制約（設問1問以上、正解の指定、ジャンル選択）で検証し、禁止語を含まないことを確認したうえで公開可視にすること。
4. 作成者がクイズを下書きまたは公開保存するとき、Quizeum Systemは指定ジャンルおよび各タグについて仮想統合マスタを参照し、統合先の正規識別子をクイズに保持すること（作成者が選択した表示用ジャンル名・タグ配列は原則変更しない）。
5. 作成者がクイズ保存時に未登録の正規化タグを含めたとき、Quizeum Systemはタグマスタに当該タグを登録し、クイズとの整合を維持すること。
6. 作成者がタグを入力したとき、Quizeum Systemはテキストを正規化（トリム、小文字化、空白・記号除去）し、類似する正規タグが既に存在する場合は警告を表示すること。
7. 作成者がクイズタイトルを編集したとき、Quizeum Systemは関連する指摘、リアクション、通知のタイトルを自動同期すること。
8. 作成者がクイズを削除したとき、Quizeum Systemは関連するブックマーク、指摘、通知、リアクションを非同期でクリーンアップまたはソフト削除すること。
9. 作成者がエクスポートを要求したとき、Quizeum Systemは当該ユーザーが作成した全クイズ（下書き含む）を単一のダウンロード可能な構造化形式にまとめること。
10. 作成者が水平思考設問を設定したとき、Quizeum Systemは視覚的なタグ風UIで真相キーワード（エッセンス）を複数追加・削除でき、文字列リストとして保持すること。
11. 水平思考設問を含むクイズを公開するとき、Quizeum Systemは水平思考設問に真相キーワードが1件以上登録されていることを検証し、未登録の場合は公開を阻止すること。
12. 作成者がテストプレイを開始するとき、Quizeum Systemはジャンル未選択を含む未完成状態でもプレイ体験を許可し、テストプレイ完了時に永続化・リーダーボード・統計更新を行わないこと（`docs` F-206 準拠）。
...
7. プレイヤーが弱点克服プレイで特定ジャンルを指定したとき、Quizeum Systemは当該ジャンルおよびマージ統合により同一カテゴリに属する関連ジャンルのクイズに限定して、過去の誤答設問を抽出すること（オールジャンル指定時は全ジャンル対象）。
...
5. マージリクエストの投票が完了し、重み付き賛成票が5以上かつ賛成率が70%以上に達したとき、Quizeum Systemは仮想統合関係をマスタに反映し、個別クイズの表示用ジャンル名・タグ名の一括物理書き換えを要求しないこと。
6. ユーザーが新規ジャンル申請（英語ID、日本語表示名、アイコン画像）を提出したとき、Quizeum Systemはモデレータ投票用のキューに載せること。
7. ユーザーがジャンル申請用アイコン画像を提出したとき、Quizeum Systemは SVG 形式を拒否し、PNG・JPEG・GIF 形式のみを受け付けること。
8. ジャンル申請が承認閾値（重み付き賛成票5以上かつ賛成率80%以上）を満たしたとき、Quizeum Systemは新規ジャンルを自動登録し、探索・作問で即時利用可能にすること。
...
#### 受け入れ基準
1. ユーザーが特定ジャンルの公開クイズ一覧を要求したとき、Quizeum Systemは当該ジャンルにマージ統合された旧ジャンルに分類されたクイズも同一一覧に含めて返すこと。
2. ユーザーが特定ジャンルの公開クイズ一覧を要求したとき、Quizeum Systemは正規ジャンル識別子に基づく一覧取得を優先し、正規識別子が未設定の既存クイズについては従来のジャンル名による結果と統合して欠落なく返すこと（canonical 検索最適化）。
3. ユーザーが特定タグの公開クイズ一覧を要求したとき、Quizeum Systemは正規タグ識別子に基づき、マージ統合前後のタグに紐づくクイズを欠落なく返すこと。
4. 認証済みユーザーまたは探索画面が有効なジャンルマスタ一覧を要求したとき、Quizeum Systemは有効なジャンルについて表示名およびアイコン表示に必要な情報を返すこと。
5. ユーザーがジャンル・難易度・問題数等の複合条件で公開クイズを検索したとき、Quizeum Systemは指定されたすべての条件に合致するクイズのみを返すこと（ジャンル条件は正規識別子およびマージ展開に整合すること）。
6. Quizeum Systemは、メタデータマスタおよびマージ申請データへの不正な書き込みを、認証およびモデレータ権限に基づき拒否すること（一般ユーザーのマスタ改ざん防止）。
7. Quizeum Systemは、重複または矛盾するジャンル新設・マージ処理の単一実装パスを維持し、同一申請に対して異なる可決結果を生まないこと。
