...
## 追加スコープ: 通報数リセット機能 — 2026-07-13

初回実装（タスク1〜10、GO判定済み）完了後、ユーザーからの追加要望により Requirement 12（ユーザー直接通報数のリセット）を追加した。

**背景**: 嫌がらせ等による組織的な大量通報（荒らし行為）を受けたユーザーが、通報ランキング上位に不当に表示され続ける問題への対処。

**スコープ確定（ユーザー回答）**:
- リセット対象は**ユーザー直接通報（`user_reports`）のみ**。クイズ通報累計（`quizzes.flags_count`）は対象外とし、既存の `quizeum-moderation-governance-ui` の審査フローとの責務分離を維持する。
- UI配置は**検索タブの詳細画面**（ティア引き下げと同じ場所）。通報ランキング一覧からの直接操作は行わない。

**設計判断**:
- 新規RPC `handle_reset_user_reports` は既存の `handle_downgrade_tier` と同型のパターン（`is_admin()`検証、理由10文字以上、`admin_logs`記録）を踏襲。
- `admin_log_action_enum` へ `report_reset` を追加する必要があり、`tier_downgrade` 追加時と同じ「同一トランザクション内で新enum値を参照できない」制約に従い、専用マイグレーションファイルとして分離する。
- `resetUserReports` は `reset`/`ban`/`downgrade-tier` と同じ「変更系アクションはAPI Route経由」パターンに従う（`unbanUser`のような直接クライアント呼び出しパターンは採用しない）。理由: このアクションは監査上重要度が高い（既存のreset/ban/downgrade-tierと同格）と判断したため、二重防御レイヤーを維持する。
- RPCは対象ユーザーに未処理の直接通報が0件でもエラーにせず冪等に振る舞う設計とし、UI側（Requirement 12.7）で事前に操作を非活性化することでUXレベルの誤操作を防止する二段構え。
