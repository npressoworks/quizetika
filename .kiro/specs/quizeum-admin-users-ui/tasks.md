...
- [x] 14.1 通報数リセットのE2E検証
  ...
  - _Boundary: TestSuite_
  - _Depends: 13.1_
  - _2人の異なるreporterから対象ユーザーへ未解決通報を2件シード（(reporter_id, target_uid)のユニーク制約を回避）。0件ユーザーのケースでは有効な理由入力後でも非活性のままであることを確認し、件数駆動の非活性化であることを証明。既存5仕様との横断実行（10/10）で相互干渉なしを確認済み。_
...
- 12.2/13.1（`/kiro-validate-impl`最終検証で発見・事後クリーンアップ）: タスク12.2で`reputation.ts`（サーバー専用クライアント版）に`getUserOpenReportCount`を実装したが、タスク13.1のレビューでUI統合はクライアント安全な`reputation-client.ts`版を使うべきと指摘され修正された結果、`reputation.ts`版は呼び出し元が存在しない孤立コードになっていた。ユーザー指摘によりサーバー版の関数・テストを削除し、design.mdの記載も訂正した。今後、同一関数をサーバー専用ファイルとクライアント安全ファイルの両方に実装する設計をする際は、実際にどちらが呼び出されるかをタスク完了時点で確認すること。
