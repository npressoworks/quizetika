...
- **OAuth（Google/X/Entra ID）の PKCE コード交換**: `signInWithGoogle` 等（`src/lib/supabase/auth.ts`）は `redirectTo` に `/api/auth/callback?redirect=<path>` を指定し、`src/app/api/auth/callback/route.ts` が `supabase.auth.exchangeCodeForSession(code)` でコード交換とログイン後リダイレクトを行う。新規 OAuth プロバイダ追加時もこのコールバックルートを再利用する。
