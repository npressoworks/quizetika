...
## Phase 44: リッチダッシュボード刷新 — フィルタリング・ドリルダウン対応（2026-07-18 ディスカバリー）

### Overview（本フェーズ）
統合ダッシュボード（`/creator/dashboard`、プレイヤー/クリエイタータブ）を、プレイヤー・クリエイター双方の目線で必要な情報を網羅し、期間・ジャンル・タグ・問題形式等でのフィルタリングと集計→明細へのドリルダウンが可能なリッチダッシュボードへ刷新する。現状はプレイヤー側がクライアントサイド集計（直近100件の `PlayHistoryEntry` + クイズメタの N+1 取得、`src/lib/player-stats.ts`）、クリエイター側が `Quiz` のキャッシュ済みカウンタ集計（`src/lib/dashboard-stats.ts`）に依存しており、全期間・任意フィルタでの集計が構造的に不可能である。

### 表示情報の精査結果（本フェーズ）

**プレイヤータブ**:

- 基本KPI（既存維持+拡張）: 累計プレイ数、平均正答率、平均解答時間、ユニーククイズ数に加え、総プレイ時間、連続プレイ日数（ストリーク）
- 推移: 期間フィルタに連動した日別/週別/月別のプレイ数・正答率トレンド（現状の直近7日固定を置換）
- 内訳分析: ジャンル別・タグ別・**問題形式別**・プレイモード別のプレイ回数と正答率（現状はジャンル/タグ上位5件のみ、形式別は未提供）
- 得意/苦手分析: 正答率上位・下位のジャンル/タグ/形式、苦手領域から復習モードへの導線
- ドリルダウン: 集計値（例: ジャンル×期間のセル）→ 該当プレイ履歴一覧 → アテンプト詳細（設問別正誤）
- ワードクラウド（Phase 42 既存、フィルタ連動化を検討）

**クリエイタータブ**:

- 基本KPI（既存維持+拡張）: 総プレイ数、ブックマーク数、平均評価、公開クイズ数に加え、ユニークプレイヤー数、期間内新規プレイ数、完走率、フォロワー数
- 推移: 期間フィルタに連動したプレイ数・ブックマーク・評価獲得のトレンド（累計値のみからの脱却）
- クイズ別パフォーマンス: プレイ数/評価/ブックマーク/平均正答率での並び替え可能なクイズランキング（期間フィルタ付き）
- ドリルダウン: クイズ → 設問別正答率・解答分布（既存の個別問題グラフを統合）・スコア分布・離脱ポイント
- 属性別比較: 自作クイズのジャンル別・問題形式別パフォーマンス
- 間違い指摘キュー（Phase 28 既存維持）

**共通フィルタ軸**: 期間（7日/30日/90日/全期間）、ジャンル、タグ、問題形式、プレイモード（プレイヤー側）、公開状態（クリエイター側）

### Approach Decision（本フェーズ）
- **Chosen**: サーバーサイド集計への移行 — Supabase RPC（または集計ビュー）でフィルタ条件付き集計を行い、UI は既存タブ構成を維持したままフィルタバー+ドリルダウンを段階的に追加
- **Why**: 現状のクライアントサイド集計（直近100件限定・N+1）では全期間・任意フィルタ集計が原理的に不可能。attempts と quizzes の JOIN 集計を DB 側で行うことで正確性とスケーラビリティを確保する
- **Rejected alternatives**:
  - クライアント側取得件数の拡大: 履歴増加に伴い破綻し、フィルタごとの再集計もクライアント負荷が過大
  - BigQuery 直接照会: レイテンシ・コスト・リアルタイム性の面でユーザー向けダッシュボードには不適（BigQuery は分析基盤としての位置づけを維持）

### Constraints（本フェーズ）
- 既存の `data-testid`・E2E（`creator-dashboard.spec.ts`）との互換性維持
- 未完了試行の記録失敗がプレイ体験を阻害しないこと（試行ライフサイクル記録はベストエフォート）

### スコープ確定事項（2026-07-18 要件フェーズ・ユーザー判断）
- **完走率・離脱ポイント**: 対応する。永続化対象プレイモードの試行記録を「開始時記録＋進行反映＋完了時更新」のライフサイクルへ拡張（quizeum-core 要件 39）。完走率・離脱分析は導入以降に開始された試行のみから算出
- **問題形式別集計の粒度**: 設問単位（解答明細に記録された `questionType`）で集計。解答時点の形式が使えるため過去データにも適用可能
- **設問別解答分布**: 累計のみ維持（期間フィルタは対象外）
- **フォロワー数**: ダッシュボードには表示しない（プロフィール画面の責務）
- **ジャンル・タグ集計**: クイズの「現在の属性」に基づく集計を許容

### Boundary Strategy（本フェーズ）
- **quizeum-creator-dash-ui（拡張）**: フィルタバー UI、ドリルダウン画面・モーダル、各セクションの表示刷新、スケルトン/空状態の一切を所有
- **quizeum-core（拡張）**: フィルタ条件付き集計サービス・Supabase RPC/ビュー・マイグレーション、`PlayerStats`/`DashboardStats` 型の後継となる集計契約を所有
- **Shared seams to watch**: 集計 RPC の入出力契約（フィルタパラメータと集計結果型）が両スペックの境界。プレイ履歴の取得契約変更が `quizeum-play-flow-ui`（履歴表示）へ波及しないこと

## Existing Spec Updates（Phase 44・依存順）
- [ ] quizeum-core -- フィルタ条件付きダッシュボード集計（RPC/ビュー・サービス・型契約）の追加。Dependencies: supabase-gameplay
- [ ] quizeum-creator-dash-ui -- フィルタバー・ドリルダウン・拡張KPI/チャートの UI 刷新。Dependencies: quizeum-core（Phase 44 拡張）
